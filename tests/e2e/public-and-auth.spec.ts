...
test("the demo page is a public product tour that leads to the free trial", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "רואים את כל החודש ובונים את הסידור במקום אחד" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "כל עובד יודע מתי הוא עובד" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "מהבקשה בטלפון לאישור המנהל" })).toBeVisible();
