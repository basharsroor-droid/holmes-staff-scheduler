...
// product name. Since F1, /demo is a public product tour, so it is crawlable
// and listed; the deleted mock-demo routes are no longer mentioned at all.

const privatePrefixes = ["/workspace", "/support", "/app", "/auth", "/api"];
const publicPaths = ["/pricing", "/demo", "/about", "/contact", "/terms", "/privacy"];
...
test("sitemap.xml lists the public pages and nothing private", async ({ request }) => {
  ...
  expect(body).not.toMatch(/<loc>[^<]*\/(workspace|support)[/<]/);
});
