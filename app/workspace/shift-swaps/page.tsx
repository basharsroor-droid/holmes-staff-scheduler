...
export default async function ShiftSwapsPage() {
  ...
  const { data: profiles } = userIds.length
    ? await supabase.from("profiles").select("id, first_name, last_name, color").in("id", userIds)
    : { data: [] };
    ...
    <main className="workspace-home" dir="rtl">
      <header className="workspace-subheader">
        <div>
          ...
          <h1>
            <Repeat2 /> החלפת משמרות
          </h1>
          <p>
            {isManager
              ? "בדיקת בקשות ואישור סופי לפני שינוי הסידור"
              : "פתיחת בקשה, אישור העובד השני ומעקב אחר החלטת המנהל"}
          </p>
        </div>
      </header>
      <ShiftSwapsClient
