...
export function ShiftSwapsClient({
  organizationId,
  currentUserId,
  isManager,
  shifts,
  ...
  const [managerNotes, setManagerNotes] = useState<Record<string, string>>(() =>
    Object.fromEntries(initialRequests.map((item) => [item.id, item.manager_note ?? ""]))
  );
  const [busy, setBusy] = useState("");
  const { message, kind, setMessage } = useStatusMessage();

  const myAssignments = assignments.filter(
    (item) => item.user_id === currentUserId && shifts.some((shift) => shift.id === item.shift_id)
  );
  ...
  const visibleRequests = isManager
    ? requests
    : requests.filter((item) => item.requested_by === currentUserId || item.target_user_id === currentUserId);
  ...
  function shiftLabel(shift: Shift | null | undefined) {
    ...
    const dayLabel = new Date(`${shift.shift_date}T12:00:00`).toLocaleDateString("he-IL", {
      weekday: "short",
      day: "numeric",
      month: "numeric"
    });
    return `${dayLabel} · ${shift.name} · ${shift.start_time.slice(0, 5)}–${shift.end_time.slice(0, 5)}`;
  }

  async function addEvent(requestId: string, action: string, note?: string | null) {
    await supabase.from("swap_request_events").insert({
  ...
  async function updateStatus(request: Request, status: SwapStatus, note?: string | null) {
    ...
    const patch = {
      ...
      decided_at: ["approved", "rejected", "cancelled"].includes(status) ? new Date().toISOString() : null,
      decided_by: ["approved", "rejected"].includes(status) ? currentUserId : null
    };
  ...
  return (
    <div className={`grid swap-workbench ${isManager ? "manager" : "employee"}`}>
      {!isManager ? (
        <section className="template-list-card swap-request-form">
          <div className="template-list-heading swap-form-heading">
            <div>
          ...
          <label className="field">
            <span>המשמרת שלי</span>
            <select
              className="input"
              value={originalAssignmentId}
              onChange={(event) => setOriginalAssignmentId(event.target.value)}
            >
              <option value="">בחירת משמרת</option>
              {myAssignments.map((assignment) => (
                <option value={assignment.id} key={assignment.id}>
                  {shiftLabel(shiftForAssignment(assignment.id))}
                </option>
              ))}
            </select>
          </label>
          <label className="field">
            <span>המשמרת המבוקשת</span>
            <select
              className="input"
              value={targetAssignmentId}
              onChange={(event) => setTargetAssignmentId(event.target.value)}
            >
              <option value="">בחירת עובד ומשמרת</option>
              {targetAssignments.map((assignment) => (
                <option value={assignment.id} key={assignment.id}>
                  {name(assignment.user_id)} · {shiftLabel(shiftForAssignment(assignment.id))}
                </option>
              ))}
            </select>
          </label>
          <label className="field">
            <span>סיבת ההחלפה</span>
            <textarea
              className="textarea"
              value={reason}
              onChange={(event) => setReason(event.target.value)}
              placeholder="למשל: מבחן באוניברסיטה"
            />
          </label>
          <button
            className="button primary"
            disabled={!!busy || !myAssignments.length}
      ...
      <section className="template-list-card swap-requests-panel">
        <div className="template-list-heading swap-list-heading">
          <div>
            <p className="eyebrow">{isManager ? "אישור מנהל" : "מעקב"}</p>
            <h2>{isManager ? "בקשות החלפה" : "הבקשות שלי"}</h2>
          </div>
          <span className="badge warning">
            <Clock3 size={15} /> {visibleRequests.filter((item) => item.status.startsWith("pending")).length} ממתינות
          </span>
        </div>
        <div className="template-list swap-request-list">
          {visibleRequests.map((request) => {
            ...
            return (
              <article className={`card swap-request-card status-${request.status}`} key={request.id}>
                <div className="mini-row swap-request-head">
                  <div className="template-icon">
                    <Repeat2 />
                  </div>
                  <span>
                    <strong>
                      {name(request.requested_by)} ↔ {name(request.target_user_id)}
                    </strong>
                    <small>
                      {shiftLabel(original)}
                      <br />
                      מול {shiftLabel(target)}
                    </small>
                  </span>
                  <span
                    className={`badge ${request.status === "approved" ? "success" : request.status === "rejected" || request.status === "cancelled" ? "critical" : "warning"}`}
                  >
                    {statusLabels[request.status]}
                  </span>
                </div>
                <p className="card-muted swap-reason">{request.reason}</p>
                {request.manager_note ? (
                  <p>
                    <strong>הערת מנהל:</strong> {request.manager_note}
                  </p>
                ) : null}
                {canEmployeeDecide ? (
                  <div className="actions">
                    <button
                      className="button primary"
                      disabled={!!busy}
                      onClick={() => void updateStatus(request, "pending_manager")}
                    >
                      <Check size={16} /> אישור והעברה למנהל
                    </button>
                    <button className="button" disabled={!!busy} onClick={() => void updateStatus(request, "rejected")}>
                      <X size={16} /> דחייה
                    </button>
                  </div>
                ) : null}
                {request.requested_by === currentUserId && request.status.startsWith("pending") ? (
                  <button className="button" disabled={!!busy} onClick={() => void updateStatus(request, "cancelled")}>
                    <X size={16} /> ביטול בקשה
                  </button>
                ) : null}
                {isManager && request.status === "pending_manager" ? (
                  <div className="grid swap-manager-decision">
                    <label className="field">
                      <span>הערת מנהל אופציונלית</span>
                      <textarea
                        className="textarea"
                        value={managerNotes[request.id] ?? ""}
                        onChange={(event) =>
                          setManagerNotes((current) => ({ ...current, [request.id]: event.target.value }))
                        }
                      />
                    </label>
                    <div className="actions">
                      <button className="button primary" disabled={!!busy} onClick={() => void approve(request)}>
                        {busy === request.id ? <Loader2 className="spin" size={16} /> : <Check size={16} />} אישור
                        והחלפה
                      </button>
                      <button
                        className="button"
                        disabled={!!busy}
                        onClick={() => void updateStatus(request, "rejected", managerNotes[request.id]?.trim() || null)}
                      >
                        <X size={16} /> דחייה
                      </button>
                    </div>
