...
export function AvailabilityClient({
  ...
  const [leaveForm, setLeaveForm] = useState<{
    ...
    endDate: string;
    note: string;
  }>({ leaveType: "vacation", startDate: "", endDate: "", note: "" });
  const [leaveBusy, setLeaveBusy] = useState(false);
  const { message: leaveMessage, kind: leaveKind, setMessage: setLeaveMessage } = useStatusMessage();
  const [choices, setChoices] = useState<Record<string, Choice>>(() => {
    const submissionIds = new Set(submissions.map((submission) => submission.id));
    return Object.fromEntries(
      entries
        .filter((entry) => submissionIds.has(entry.submission_id))
        .map((entry) => [
          `${entry.shift_date}|${entry.shift_template_id}`,
          { status: entry.status, note: entry.note ?? "" }
        ])
    );
  });
  const [submittedByPeriod, setSubmittedByPeriod] = useState<Record<string, string | null>>(() =>
    Object.fromEntries(submissions.map((item) => [item.schedule_period_id, item.submitted_at]))
  );
  ...
  const days = useMemo(
    () =>
      period ? Array.from({ length: new Date(period.year, period.month, 0).getDate() }, (_, index) => index + 1) : [],
    [period]
  );
  ...
  function updateChoice(key: string, patch: Partial<Choice>) {
    setChoices((current) => ({
      ...current,
      [key]: { status: current[key]?.status ?? "", note: current[key]?.note ?? "", ...patch }
    }));
  }
  ...
  async function ensureSubmission() {
    ...
    submissions.push({
      ...
      manager_note: existingRow.manager_note
    });
  ...
  async function save(finalSubmit: boolean) {
    ...
    const rows = selected.map(([key, choice]) => {
      ...
      return {
        ...
        shift_template_id,
        shift_date,
        status: choice.status as AvailabilityStatus,
  ...
  const leaveCard = (
    <section className="template-form-card availability-leave-card">
      <div>
        <p className="eyebrow">חופשה ומחלה</p>
      ...
      <div className="form-pair">
        <label className="field">
          ...
          <select
            ...
            onChange={(e) => setLeaveForm((c) => ({ ...c, leaveType: e.target.value as LeaveType }))}
          >
        ...
        <label className="field">
          ...
          <input
            ...
            onChange={(e) => setLeaveForm((c) => ({ ...c, note: e.target.value }))}
          />
        </label>
      </div>
      <div className="form-pair">
        <label className="field">
          ...
          <input
            ...
            onChange={(e) => setLeaveForm((c) => ({ ...c, startDate: e.target.value }))}
          />
        </label>
        <label className="field">
          <span>עד תאריך</span>
          <input
            ...
            onChange={(e) => setLeaveForm((c) => ({ ...c, endDate: e.target.value }))}
          />
        </label>
      </div>
      <button className="button primary" disabled={leaveBusy} onClick={() => void addLeaveRequest()}>
        {leaveBusy ? <Loader2 className="spin" size={17} /> : <Palmtree size={17} />} שליחת בקשה
      </button>
      <StatusMessage message={leaveMessage} kind={leaveKind} />
      {leaveRequests.length ? (
        <div className="template-list">
          {leaveRequests.map((item) => (
            <article className="template-item" key={item.id}>
              <div className="template-main">
                <strong>{leaveTypeLabels[item.leave_type]}</strong>
                <span>
                  {new Date(`${item.start_date}T12:00:00`).toLocaleDateString("he-IL")} –{" "}
                  {new Date(`${item.end_date}T12:00:00`).toLocaleDateString("he-IL")}
                  {item.note ? ` · ${item.note}` : ""}
                </span>
              </div>
              <button className="button danger" disabled={leaveBusy} onClick={() => void removeLeaveRequest(item.id)}>
                <Trash2 size={15} /> ביטול
              </button>
            </article>
          ))}
        </div>
  ...
  return (
    <div className="template-workbench availability-workbench">
      ...
      <section className="template-list-card availability-form-panel">
        <div className="template-list-heading availability-toolbar">
          <div>
            <p className="eyebrow">תקופת הגשה</p>
            <h2>{period ? `${monthNames[period.month - 1]} ${period.year}` : ""}</h2>
          </div>
          <select
            className="input availability-period-select"
            aria-label="בחירת חודש הגשה"
        ...
        {period ? (
          <div className={`submission-banner availability-deadline ${deadlinePassed || notOpened ? "closed" : "open"}`}>
            <Clock3 />
            <div>
              <strong>
                {notOpened
        ...
        <div className="availability-board availability-selection-board">
          {days.map((day) => {
            ...
            const label = new Date(`${date}T12:00:00`).toLocaleDateString("he-IL", {
              weekday: "long",
              day: "numeric",
              month: "numeric"
            });
            return (
              <article className="availability-card availability-day-card" key={date}>
                <strong>{label}</strong>
                {periodTemplates.map((template) => {
                  const key = `${date}|${template.id}`;
                  const choice = choices[key] ?? { status: "", note: "" };
                  return (
                    <div className="card-muted availability-shift-choice" key={template.id}>
                      ...
                      <select
                        className="input"
                        disabled={deadlinePassed || notOpened}
                        aria-label={`זמינות ל${label}, משמרת ${template.name}`}
                        value={choice.status}
                        onChange={(e) => updateChoice(key, { status: e.target.value as AvailabilityStatus | "" })}
                      >
                      ...
                      {choice.status ? (
                        <input
                          ...
                          aria-label={`הערה עבור ${label}, משמרת ${template.name}`}
                          value={choice.note}
                          onChange={(e) => updateChoice(key, { note: e.target.value })}
                        />
                      ) : null}
                    </div>
                  );
        ...
        <div className="actions availability-submit-actions">
          <button className="button" disabled={busy || deadlinePassed || notOpened} onClick={() => void save(false)}>
            {busy ? <Loader2 className="spin" size={17} /> : <Save size={17} />} שמירת טיוטה
          </button>
          <button
            className="button primary"
            disabled={busy || deadlinePassed || notOpened}
          ...
          >
            ...
            {submittedByPeriod[period?.id ?? ""] ? "עדכון ושליחה מחדש" : "שליחה למנהל"}
          </button>
        </div>
