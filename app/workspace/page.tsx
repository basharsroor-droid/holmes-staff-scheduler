...
import { Activity, Bell, Building2, CalendarCheck, CalendarDays, CalendarRange, CheckCircle2, Circle, ClipboardCheck, Clock3, History, LifeBuoy, MessageSquareText, Network, Repeat2, Rocket, Search, Settings, ShieldCheck, Users } from "lucide-react";
...
export default async function WorkspacePage() {
  ...
  const pilotSteps = [
    { complete: (templatesResult.count ?? 0) > 0, href: "/workspace/shift-templates", title: "הגדרת סוגי משמרות", description: "הוסיפו פתיחה, אמצע וסגירה עם השעות והתקן האמיתיים." },
    { complete: (periodsResult.count ?? 0) > 0, href: "/workspace/work-months", title: "פתיחת חודש עבודה", description: "בחרו חודש וקבעו מתי העובדים יכולים להגיש זמינות." },
    { complete: (membersResult.count ?? 0) > 1, href: "/workspace/employees", title: "הזמנת צוות הפיילוט", description: "הזמינו מנהל/ת ולפחות שני עובדי בדיקה במייל." }
  ];
  ...
  return <main className="workspace-home" dir="rtl">
    ...
    {isEmployee ? <>
      ...
      {completedPilotSteps < pilotSteps.length ? <section className="pilot-readiness" aria-labelledby="pilot-readiness-title">
        <div className="pilot-readiness-head"><div><p className="eyebrow">הכנת הפיילוט</p><h2 id="pilot-readiness-title">שלושה צעדים עד להגשת זמינות ראשונה</h2><p>השלימו את ההגדרות לפי הסדר. הנתונים נשמרים רק בעסק ובסניף שלכם.</p></div><div className="pilot-progress" aria-label={`${pilotProgress}% הושלמו`}><strong>{pilotProgress}%</strong><small>{completedPilotSteps} מתוך {pilotSteps.length}</small></div></div>
        <div className="pilot-progress-track" aria-hidden="true"><span style={{ width: `${pilotProgress}%` }} /></div>
        <div className="pilot-checklist">{pilotSteps.map((step, index) => <Link className={step.complete ? "complete" : ""} href={step.href} key={step.href}>{step.complete ? <CheckCircle2 aria-hidden="true" /> : <Circle aria-hidden="true" />}<span><small>שלב {index + 1}</small><strong>{step.title}</strong><em>{step.description}</em></span><b>{step.complete ? "הושלם" : "המשך"}</b></Link>)}</div>
      </section> : <section className="pilot-readiness pilot-ready"><div><CheckCircle2 aria-hidden="true" /><span><p className="eyebrow">מוכנים לבדיקה</p><h2>הגדרות הפיילוט הושלמו</h2><p>אפשר להתחיל בהגשת זמינות, לבדוק את ההגשות ולבנות סידור ראשון.</p></span></div><Link className="button primary" href="/workspace/submissions">מעקב הגשות</Link></section>}
