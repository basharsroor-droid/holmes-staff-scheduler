...
export function EmployeePreferenceEnhancer() {
  useEffect(() => {
    ...
    function guardFallback(event: MouseEvent) {
      ...
      if (
        !button ||
        preferenceLabel(button) !== "רק אם צריך" ||
        button.disabled ||
      ...
      const strongerCandidateExists = Array.from(
        grid.querySelectorAll<HTMLButtonElement>("button.schedule-worker")
      ).some(
        (candidate) =>
          candidate !== button &&
          !candidate.disabled &&
    ...
    <>
      <section className="preference-decision-guide" aria-label="סדר עדיפויות לשיבוץ">
        ...
        <div className="preference-decision-items">
          <span>
            <Heart size={16} /> <b>מועדפת</b>
            <small>בחירה ראשונה</small>
          </span>
          <span>
            <UserCheck size={16} /> <b>זמין/ה</b>
            <small>מתאים לשיבוץ</small>
          </span>
          <span>
            <ShieldAlert size={16} /> <b>רק אם צריך</b>
            <small>גיבוי בלבד</small>
          </span>
          <span>
            <UserRoundX size={16} /> <b>לא זמין/ה</b>
            <small>חסום לשיבוץ</small>
          </span>
        </div>
      </section>
      ...
      <style jsx global>{`
        ...
        .preference-decision-guide h2 {
          margin: 2px 0 6px;
          font-size: 1.1rem;
        }
        .preference-decision-guide p {
          margin: 0;
          color: var(--muted);
        }
        .preference-decision-items {
          display: grid;
          grid-template-columns: repeat(2, minmax(0, 1fr));
          gap: 8px;
        }
        .preference-decision-items span {
          display: grid;
        ...
        .preference-decision-items small {
          grid-column: 2;
          color: var(--muted);
        }
        .schedule-worker-grid {
          display: flex !important;
          flex-wrap: wrap;
        }
        .schedule-worker[data-preference="preferred"] {
          box-shadow: inset 0 0 0 2px color-mix(in srgb, var(--primary) 42%, transparent);
        }
        .schedule-worker[data-preference="preferred"] small::before {
          content: "♥ ";
        }
        .schedule-worker[data-preference="fallback"] {
          opacity: 0.82;
          border-style: dashed;
        }
        .schedule-worker[data-preference="unavailable"],
        .schedule-worker[data-preference="leave"] {
          opacity: 0.55;
        }
