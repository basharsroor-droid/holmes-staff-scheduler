...
export function CoverageRulesEnhancer({
  workers,
  templates
}: {
  ...
  const seniorNames = useMemo(
    () =>
      new Set(
        workers
          .filter((worker) => isSenior(worker.seniority_level))
          .map((worker) =>
            worker.profile ? normalizedName(`${worker.profile.first_name} ${worker.profile.last_name}`) : ""
          )
          .filter(Boolean)
      ),
    [workers]
  ...
  const seniorRequiredNames = useMemo(
    () =>
      new Set(
        templates
          .filter((template) => template.requires_senior_employee)
          .map((template) => normalizedName(template.name))
      ),
    [templates]
  );
    ...
    function calculate() {
      ...
      cards.forEach((card, index) => {
        ...
        const selectedWorkers = Array.from(
          card.querySelectorAll<HTMLButtonElement>("button.schedule-worker[aria-pressed='true']")
        );
    ...
    function guardPublish(event: MouseEvent) {
      ...
      cardsNow.forEach((card) => {
        ...
        if (seniorRequiredNames.has(title)) {
          ...
          const hasSenior = selectedWorkers.some((workerButton) =>
            seniorNames.has(normalizedName(workerButton.querySelector("span")?.textContent ?? ""))
          );
  ...
  return (
    <section className={`coverage-rules-panel ${issues.length ? "has-issues" : "all-clear"}`} aria-live="polite">
      <div className="coverage-rules-head">
        <div>
          ...
          <h2>
            {issues.length ? (
              <>
                <ShieldAlert size={20} /> חריגי כיסוי לפני פרסום
              </>
            ) : (
              <>
                <CheckCircle2 size={20} /> הכיסוי תקין
              </>
            )}
          </h2>
          <p>
        ...
        <div className="coverage-rules-summary">
          <span>
            <Users size={16} />
            <strong>{missingStaffCount}</strong>
            <small>חוסר בכוח אדם</small>
          </span>
          <span>
      ...
      <style jsx global>{`
        ...
        .coverage-rules-panel.has-issues {
          border-style: dashed;
        }
        .coverage-rules-head {
          display: flex;
        ...
        .coverage-rules-head h2 {
          display: flex;
          gap: 8px;
          align-items: center;
          margin: 4px 0 6px;
        }
        .coverage-rules-head p {
          margin: 0;
        }
