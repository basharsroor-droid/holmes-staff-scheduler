...
export function TimeOffApprovalPanel({ initialRequests }: { initialRequests: PendingRequest[] }) {
  ...
  return (
    <section className="template-list-card" style={{ marginBottom: 20 }}>
      <div className="template-list-heading">
        <div>
          <p className="eyebrow">Time Off</p>
          <h2>
            <Palmtree size={20} /> בקשות חופשה שממתינות לאישור
          </h2>
        </div>
        <span className={`badge ${requests.length ? "warning" : "opening"}`}>
          <Clock3 size={15} /> {requests.length} ממתינות
        </span>
      </div>
          ...
          {requests.map((request) => (
            <article className="template-item" key={request.id}>
              <div className="template-main">
                <strong>
                  {request.employee_name} · {leaveTypeLabels[request.leave_type]}
                </strong>
                <span>
