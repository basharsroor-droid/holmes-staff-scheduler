...
export function OpenShiftsManagerPanel({
  initialShifts,
  initialRequests
}: {
  ...
  async function setOpen(shiftId: string, makeOpen: boolean) {
    setBusy(`open-${shiftId}`);
    setMessage("");
    const { error } = await supabase.rpc("set_shift_open_for_requests", {
          ...
          return (
            <article className="card-muted" key={shift.id}>
              <div className="shift-title">
                ...
                <small>
                  {shift.period_label} · {shift.assigned_count}/{shift.required_employees}
                </small>
              </div>
              <p>
              ...
              <div className="actions">
                <button
                  className={shift.open_for_requests ? "button danger" : "button primary"}
                  disabled={!!busy}
                  onClick={() => void setOpen(shift.id, !shift.open_for_requests)}
                >
                  {busy === `open-${shift.id}` ? (
                  ...
                  {shift.open_for_requests ? " סגירת בקשות" : " פתיחה לעובדים"}
                </button>
              ...
              {shiftRequests.length ? (
                <div className="grid">
                  {shiftRequests.map((request) => (
                    <div className="card-muted" key={request.id}>
                      <strong>{request.employee_name}</strong>
                      <small>בקשה מ־{new Date(request.created_at).toLocaleString("he-IL")}</small>
                      <div className="actions">
                        <button
                        ...
                        >
                          <CheckCircle2 size={15} /> אישור ושיבוץ
                        </button>
                        <button
                          className="button"
                          disabled={!!busy}
