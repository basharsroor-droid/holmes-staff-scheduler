...
export function ScheduleBuilderClient({
  ...
  const periodSubmissions = submissions.filter(
    (item) => item.schedule_period_id === selectedPeriodId && item.submitted_at
  );
  const days = period
    ? Array.from({ length: new Date(period.year, period.month, 0).getDate() }, (_, index) =>
        dateKey(period.year, period.month, index + 1)
      )
    : [];
  const holidaysByDate = period
    ? getIsraeliHolidaysForMonth(period.year, period.month)
    : new Map<string, IsraeliHoliday>();
  const filled = periodShifts.filter(
    (shift) => assignments.filter((item) => item.shift_id === shift.id).length >= shift.required_employees
  ).length;
  ...
  function hasConflict(userId: string, shift: Shift) {
    return activeShifts.some(
      (other) =>
        ...
        assignments.some((item) => item.shift_id === other.id && item.user_id === userId) &&
  ...
  function workerLeave(userId: string, shift: Shift) {
    return (
      leaveRequests.find(
        (item) => item.user_id === userId && shift.shift_date >= item.start_date && shift.shift_date <= item.end_date
      ) ?? null
    );
  }
  ...
  function weeklyHours(userId: string, weekKey: string) {
    return activeShifts
      .filter(
        (item) =>
          weekStartKey(item.shift_date) === weekKey &&
          assignments.some((a) => a.shift_id === item.id && a.user_id === userId)
      )
  ...
  async function generateMonth() {
    ...
    setBusy("generate");
    setMessage("");
    const rows = days.flatMap((shiftDate) =>
    ...
    const { data, error } = await supabase
      .from("shifts")
      .upsert(rows, { onConflict: "schedule_period_id,shift_date,name" })
      .select(
  ...
  async function duplicateFromPrevious() {
    ...
    setAssignments((current) => [
      ...current.filter((item) => !newShiftIds.includes(item.shift_id)),
      ...(newAssignments ?? [])
    ]);
    setMessage(
      `שוכפלו ${result.shifts_created} משמרות ו-${result.assignments_created} שיבוצים מהחודש שנבחר` +
        (result.assignments_skipped_inactive
          ? ` (${result.assignments_skipped_inactive} שיבוצים דולגו כי העובד/ת כבר לא פעיל/ה בארגון)`
          : "") +
        "."
    );
  }
  ...
  async function cancelDay(date: string) {
    ...
    setBusy(`cancel-${date}`);
    setMessage("");
    const { data, error } = await supabase.rpc("cancel_shifts_for_day", {
      target_period_id: period.id,
      target_date: date
    });
    setBusy("");
    ...
    const cancelledIds = new Set(
      shifts.filter((item) => item.schedule_period_id === period.id && item.shift_date === date).map((item) => item.id)
    );
    setShifts((current) =>
  ...
  async function assign(shift: Shift, userId: string) {
    ...
    if (limit) {
      ...
      if (
        projected > limit &&
        !window.confirm(
          `${workerName(userId)} יעבור/תעבור את מגבלת ${limit} השעות השבועית שלו/ה (יגיע/תגיע ל-${Math.round(projected * 10) / 10} שעות). לשבץ בכל זאת?`
        )
      ) {
        setBusy("");
        return;
    ...
    if (minRestHours) {
      ...
      if (
        gap < minRestHours &&
        !window.confirm(
          `יהיו רק ${Math.round(gap * 10) / 10} שעות מנוחה בין המשמרות של ${workerName(userId)} (מגבלה: ${minRestHours} שעות). לשבץ בכל זאת?`
        )
      ) {
        setBusy("");
        return;
    ...
    const { data, error } = await supabase
      ...
      .insert({ organization_id: organizationId, shift_id: shift.id, user_id: userId, assigned_by: currentUserId })
  ...
  async function publish() {
    ...
    const missing = periodShifts.filter(
      (shift) => assignments.filter((item) => item.shift_id === shift.id).length < shift.required_employees
    ).length;
    ...
    for (const date of days) {
      ...
      for (const shift of dayShifts) {
        const assignedNames = assignments
          ...
          .map((item) => workerName(item.user_id));
  ...
  return (
    <section className="template-list-card schedule-workbench">
      <div className="no-print">
        <div className="template-list-heading schedule-toolbar">
          <div>
            <p className="eyebrow">חודש, סניף ומחלקה</p>
            <h2>{period ? `${monthNames[period.month - 1]} ${period.year}` : ""}</h2>
            <span className={`schedule-state ${periodStatus === "published" ? "published" : "draft"}`}>
              {periodStatus === "published" ? "פורסם לצוות" : "טיוטה בעבודה"}
            </span>
          </div>
          <select
          ...
          >
            {periods.map((item) => (
              <option value={item.id} key={item.id}>
                ...
                {departments.find((department) => department.id === item.department_id)?.name ?? "מחלקה"}
              </option>
            ))}
          </select>
        </div>
        <div className="workspace-stats schedule-stats">
          <article>
            <CalendarRange />
            <span>
              <strong>{periodShifts.length}</strong>
              <small>משמרות בחודש</small>
            </span>
          </article>
          <article>
            <CheckCircle2 />
            <span>
              ...
              <small>משמרות מאוישות · {completion}%</small>
            </span>
          </article>
          <article>
            <UserPlus />
            <span>
              <strong>{periodWorkers.length}</strong>
              <small>עובדים לשיבוץ</small>
            </span>
          </article>
        </div>
        <div className="actions schedule-primary-actions">
          <button
            className="button primary"
            disabled={!!busy || !periodTemplates.length}
            onClick={() => void generateMonth()}
          >
            {busy === "generate" ? <Loader2 className="spin" size={16} /> : <Sparkles size={16} />}{" "}
            {periodShifts.length ? "סנכרון משמרות החודש" : "יצירת משמרות החודש"}
          </button>
          {periodStatus === "published" ? (
            <button className="button" disabled={!!busy} onClick={() => void unpublish()}>
              {busy === "unpublish" ? <Loader2 className="spin" size={16} /> : <Undo2 size={16} />} ביטול פרסום
            </button>
          ) : (
            <button className="button" disabled={!!busy || !periodShifts.length} onClick={() => void publish()}>
              {busy === "publish" ? <Loader2 className="spin" size={16} /> : <Send size={16} />} פרסום הסידור
            </button>
          )}
          {periodShifts.length ? (
        ...
        {!periodShifts.length && duplicateCandidates.length ? (
          <div className="actions schedule-duplicate-actions">
            <select
              className="input"
              style={{ maxWidth: 260 }}
              aria-label="שכפול משמרות ושיבוצים מחודש קודם"
            ...
            <button
              className="button"
              disabled={!!busy || !duplicateSourceId}
              onClick={() => void duplicateFromPrevious()}
            >
              {busy === "duplicate" ? <Loader2 className="spin" size={16} /> : <CopyPlus size={16} />} שכפול משמרות
              ושיבוצים
            </button>
          </div>
        ) : null}
        {callerRole === "owner" || callerRole === "admin" ? (
          <label className="field schedule-rest-field">
            <span>מגבלת שעות מנוחה מינימלית בין משמרות</span>
            <input
              className="input"
              type="number"
              min={1}
              ...
              value={minRestDraft}
              onChange={(event) => setMinRestDraft(event.target.value)}
              onBlur={() => void saveMinRestHours()}
            />
          </label>
        ) : null}
        {!periodTemplates.length ? (
          <div className="submission-banner closed">
            <AlertTriangle />
            <div>
        ...
        <div className="availability-board schedule-board">
          {days.map((date) => {
            const dayShifts = periodShifts.filter((shift) => shift.shift_date === date);
            if (!dayShifts.length) return null;
            const holiday = holidaysByDate.get(date);
            return (
              <article className="availability-card schedule-day-card" key={date}>
                <div className="shift-title schedule-day-head">
                  <span className="schedule-day-label">
                    ...
                    {holiday ? (
                      <span className={`badge ${holidayBadgeClass[holiday.kind]}`}>{holiday.label}</span>
                    ) : null}
                  </span>
                  <button
                    type="button"
                    ...
                    disabled={!!busy}
                    onClick={() => void cancelDay(date)}
                  >
                    {busy === `cancel-${date}` ? <Loader2 className="spin" size={14} /> : <Trash2 size={14} />} ביטול
                    משמרות היום
                  </button>
                </div>
                {dayShifts.map((shift) => {
                  ...
                  return (
                    <div className="card-muted schedule-shift-card" key={shift.id}>
                      <div className="shift-title">
                        <span>{shift.name}</span>
                        <small>
                          {shift.start_time.slice(0, 5)}–{shift.end_time.slice(0, 5)} · {assigned.length}/
                          {shift.required_employees}
                        </small>
                      </div>
                      <div className="grid schedule-worker-grid">
                        {periodWorkers.map((worker) => {
                          ...
                          return (
                            <button
                              type="button"
                              className={`button schedule-worker ${selected ? "primary" : ""}`}
                              ...
                              onClick={() => void assign(shift, worker.user_id)}
                              key={worker.user_id}
                            >
                              <span>{workerName(worker.user_id)}</span>
                              <small>
                                {leave
      ...
      {period ? (
        <div className="print-only">
          <h2>
            ...
            {branches.find((branch) => branch.id === period.branch_id)?.name ?? "סניף"} ·{" "}
            {departments.find((department) => department.id === period.department_id)?.name ?? "מחלקה"}
          </h2>
          {days.map((date) => {
            const dayShifts = periodShifts.filter((shift) => shift.shift_date === date);
            if (!dayShifts.length) return null;
            return (
              <div className="print-schedule-day" key={date}>
                <h3>
                  {new Date(`${date}T12:00:00`).toLocaleDateString("he-IL", {
                    weekday: "long",
                    day: "numeric",
                ...
                {dayShifts.map((shift) => {
                  const assignedNames = assignments
                    ...
                    .map((item) => workerName(item.user_id));
                  return (
                    <div className="print-schedule-shift" key={shift.id}>
                      <span>
                        <strong>{shift.name}</strong> {shift.start_time.slice(0, 5)}–{shift.end_time.slice(0, 5)}
                      </span>
