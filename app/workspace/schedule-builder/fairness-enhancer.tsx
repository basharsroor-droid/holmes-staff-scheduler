          ...
          {findings.map((finding) => (
            <article className="card" key={finding.key}>
              <div className="mini-row">
                ...
                <span className={`badge ${finding.severity === "warning" ? "warning" : "opening"}`}>
                  {finding.severity === "warning" ? "לבדיקה" : "מידע"}
                </span>
              </div>
            </article>
          ))}
      ...
      {metrics.length ? (
        <div className="template-list" style={{ marginTop: 12 }}>
          {metrics.slice(0, 8).map((metric) => (
            <article className="card" key={metric.userId}>
              <div className="mini-row">
                <span>
                  <strong>{metric.name}</strong>
                  <small>
                    {metric.assignedHours} שעות · {metric.preferredAssigned}/{metric.preferredOpportunities} העדפות
                    מומשו · {metric.onlyIfNeededAssigned} ‘רק אם צריך’
                  </small>
                </span>
                <span className="badge opening">{metric.eligibleOpportunities} הזדמנויות</span>
              </div>
            </article>
          ))}
        </div>
