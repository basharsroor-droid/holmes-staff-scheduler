...
export function SmartReplacementPanel({
  ...
  const rank = useCallback(async () => {
    ...
    const weeklyHours = (userId: string) =>
      allShifts
        .filter(
          (s) =>
            weekStartKey(s.shift_date) === week &&
            allAssignments.some((a) => a.shift_id === s.id && a.user_id === userId)
        )
        .reduce((sum, s) => sum + shiftHours(s), 0);
    const totalHours = (userId: string) =>
      allShifts
        .filter((s) => allAssignments.some((a) => a.shift_id === s.id && a.user_id === userId))
        .reduce((sum, s) => sum + shiftHours(s), 0);
    const minGap = (userId: string) => {
      const c = shiftBounds(shift);
      let gap = Infinity;
      for (const other of allShifts) {
        if (other.id === shift.id || !allAssignments.some((a) => a.shift_id === other.id && a.user_id === userId))
          continue;
    ...
    const hasOtherSenior = otherAssignedUserIds.some(
      (id) => periodWorkers.find((w) => w.user_id === id)?.seniority_level === "senior"
    );
    ...
    const ranked = periodWorkers
      .map((worker) => {
        if (worker.user_id === outgoingUserId || targetAssignments.some((a) => a.user_id === worker.user_id))
          return null;
        ...
        if (
          approvedLeave.some(
            (l) => l.user_id === worker.user_id && shift.shift_date >= l.start_date && shift.shift_date <= l.end_date
          )
        )
          return null;
        if (
          allShifts.some(
            (other) =>
              other.id !== shift.id &&
              allAssignments.some((a) => a.shift_id === other.id && a.user_id === worker.user_id) &&
              shiftsOverlap(other, shift)
          )
        )
          return null;
        if (worker.weekly_hours_limit && weeklyHours(worker.user_id) + shiftHours(shift) > worker.weekly_hours_limit)
          return null;
  ...
  const apply = useCallback(
    async (candidate: Candidate) => {
      ...
      if (period.status === "published") {
        setCandidates([]);
        setMessage("לא ניתן לבצע Smart Replacement על סידור שפורסם. יש לבטל פרסום ולדרג מחדש.");
        return;
      }
      if (
        !window.confirm(
      ...
      if (!currentPeriod || currentPeriod.status === "published") {
        setBusy("");
        setCandidates([]);
        setMessage("התקופה פורסמה מאז הדירוג. לא בוצע שינוי; בטל פרסום ודרג מחדש.");
        return;
      }
      ...
      if (!currentShift || currentShift.schedule_period_id !== selectedPeriodId) {
        setBusy("");
        setCandidates([]);
        setMessage("המשמרת השתנתה מאז הדירוג. לא בוצע שינוי; יש לטעון ולדרג מחדש.");
        return;
      }
      const allIds = allShifts.map((s) => s.id);
      const { data: allAssignmentRows } = allIds.length
      ...
      if (
        !targetAssignments.some((a) => a.user_id === outgoingUserId) ||
        targetAssignments.some((a) => a.user_id === candidate.userId)
      ) {
        setBusy("");
        setCandidates([]);
        setMessage("השיבוץ השתנה מאז הדירוג. לא בוצע שינוי; יש לחשב מועמדים מחדש.");
        return;
      }
      ...
      if (!candidateWorker || !candidateWorker.department_ids.includes(period.department_id)) {
        setBusy("");
        setCandidates([]);
        setMessage("המועמד כבר לא שייך למחלקה המתאימה. לא בוצע שינוי; יש לדרג מחדש.");
        return;
      }
      const sub = submissions.find(
        (s) => s.schedule_period_id === selectedPeriodId && s.user_id === candidate.userId && s.submitted_at
      );
      const status =
        sub && currentShift.shift_template_id
          ? (availability.find(
              (a) =>
                a.submission_id === sub.id &&
                a.shift_date === currentShift.shift_date &&
                a.shift_template_id === currentShift.shift_template_id
            )?.status ?? null)
          : null;
      if (
        !status ||
        ...
        approvedLeave.some(
          (l) =>
            l.user_id === candidate.userId &&
            currentShift.shift_date >= l.start_date &&
            currentShift.shift_date <= l.end_date
        )
      ) {
        setBusy("");
        setCandidates([]);
        setMessage("זמינות המועמד או Time Off כבר לא מאפשרים את ההחלפה. לא בוצע שינוי; יש לדרג מחדש.");
        return;
      }
      if (
        allShifts.some(
          (other) =>
            other.id !== currentShift.id &&
            allAssignments.some((a) => a.shift_id === other.id && a.user_id === candidate.userId) &&
            shiftsOverlap(other, currentShift)
        )
      ) {
        setBusy("");
        setCandidates([]);
        setMessage("למועמד נוצרה חפיפה מאז הדירוג. לא בוצע שינוי; יש לדרג מחדש.");
        return;
      }
      const week = weekStartKey(currentShift.shift_date);
      const currentWeeklyHours = allShifts
        .filter(
          (s) =>
            weekStartKey(s.shift_date) === week &&
            allAssignments.some((a) => a.shift_id === s.id && a.user_id === candidate.userId)
        )
        .reduce((sum, s) => sum + shiftHours(s), 0);
      if (
        candidateWorker.weekly_hours_limit &&
        currentWeeklyHours + shiftHours(currentShift) > candidateWorker.weekly_hours_limit
      ) {
        setBusy("");
        setCandidates([]);
        setMessage("החלפה תחרוג ממכסת השעות השבועית של המועמד. לא בוצע שינוי; יש לדרג מחדש.");
        return;
      }
      if (minRestHours) {
        const c = shiftBounds(currentShift);
        let gap = Infinity;
        for (const other of allShifts) {
          if (
            other.id === currentShift.id ||
        ...
        if (gap < minRestHours) {
          setBusy("");
          setCandidates([]);
          setMessage("מנוחת המינימום של המועמד כבר לא נשמרת. לא בוצע שינוי; יש לדרג מחדש.");
          return;
        }
      }
      ...
      const outgoingIsSenior = periodWorkers.find((w) => w.user_id === outgoingUserId)?.seniority_level === "senior";
      if (
        template?.requires_senior_employee &&
        outgoingIsSenior &&
        !hasOtherSenior &&
        candidateWorker.seniority_level !== "senior"
      ) {
        setBusy("");
        setCandidates([]);
        setMessage("ההחלפה תפגע בכיסוי Senior שנדרש למשמרת. לא בוצע שינוי; יש לדרג מחדש.");
        return;
      }
      ...
      const { error: replaceError } = await db.rpc("replace_shift_assignment", {
        target_shift_id: currentShift.id,
        outgoing_user_id: outgoingUserId,
        incoming_user_id: candidate.userId
      });
      if (replaceError) {
        setBusy("");
        setCandidates([]);
      ...
      <div className="form-grid" style={{ marginTop: 12 }}>
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => {
              setOutgoingUserId(e.target.value);
              setCandidates([]);
              setMessage("");
            }}
      ...
      {candidates.length ? (
        <div className="template-list" style={{ marginTop: 12 }}>
          {candidates.slice(0, 10).map((candidate, index) => (
            <article className="card" key={candidate.userId}>
              <div className="mini-row">
                <span>
                  <strong>
                    #{index + 1} · {candidate.name} · ציון {candidate.score}
                  </strong>
