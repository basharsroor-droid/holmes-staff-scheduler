...
export function ScheduleTemplatesPanel({
  ...
  const applicableTemplates = templates.filter(
    (t) => !target || (t.branch_id === target.branch_id && t.department_id === target.department_id)
  );
  ...
  return (
    <section className="template-list-card" dir="rtl">
      ...
      <div className="grid grid-2" style={{ alignItems: "end" }}>
            ...
            {periods
              ...
              .map((p) => (
                <option key={p.id} value={p.id}>
                  {monthNames[p.month - 1]} {p.year} · {p.shift_count} משמרות
                </option>
              ))}
          </select>
        </label>
        <label>
          ...
          <input
            ...
            placeholder="למשל: חודש רגיל - קבלה"
            maxLength={120}
          />
        </label>
      </div>
      ...
      <div className="grid grid-2" style={{ alignItems: "end" }}>
        <label>
          ...
          <select
            ...
            onChange={(e) => {
              setTargetPeriodId(e.target.value);
              setTemplateId("");
            }}
          >
          ...
          {templates.map((t) => (
            ...
            >
              <div>
                ...
                <p style={{ margin: "4px 0 0" }}>{t.item_count} משמרות בתבנית</p>
              </div>
              <button className="button" disabled={!!busy} onClick={() => void removeTemplate(t.id)}>
                {busy === `delete:${t.id}` ? <Loader2 className="spin" size={16} /> : <Trash2 size={16} />} מחיקה
              </button>
            </div>
          ))}
        </div>
