...
export function SupportClient({
  ...
  const [notes, setNotes] = useState<Record<string, string>>(() =>
    Object.fromEntries(initialTickets.map((ticket) => [ticket.id, ticket.resolution_note ?? ""]))
  );
  ...
  async function updateTicket(ticket: Ticket, status: TicketStatus) {
    ...
    const { error } = await supabase
      .from("support_tickets")
      .update({ status, resolution_note: resolutionNote, assigned_to: currentUserId })
      .eq("id", ticket.id);
    setBusy("");
    if (error) {
  ...
  return (
    <div
      className={`template-workbench support-workbench support-experience${showCreateForm ? "" : " support-console-workbench"}`}
    >
      {showCreateForm ? (
        <section className="template-form-card support-create-card">
          <div>
          ...
          <label className="field">
            ...
            <input
              ...
              placeholder="למשל: לא ניתן לפרסם את הסידור"
            />
          </label>
          ...
          <button className="button primary" disabled={!!busy} onClick={() => void createTicket()}>
            {busy === "create" ? <Loader2 className="spin" size={16} /> : <Send size={16} />} פתיחת פנייה
          </button>
          <StatusMessage message={message} kind={kind} />
        </section>
      ) : null}
      <section className="template-list-card support-tickets-panel">
        <div className="template-list-heading support-list-heading">
          <div>
            <p className="eyebrow">מעקב שקוף</p>
            <h2>{canManage ? "פניות העסק" : "הפניות שלי"}</h2>
          </div>
          <span className="badge warning">
            <Clock3 size={15} /> {tickets.filter((ticket) => unresolvedStatuses.includes(ticket.status)).length} פתוחות
          </span>
        </div>
        <div className="support-filter-tabs" role="tablist">
          {(Object.keys(filterLabels) as TicketFilter[]).map((key) => (
            <button
              key={key}
              type="button"
              role="tab"
        ...
        <div className="template-list support-ticket-list">
          {tickets
            .filter((ticket) => matchesFilter(ticket, filter))
            .map((ticket) => (
              <article
                className={`card support-ticket priority-${ticket.priority} status-${ticket.status}`}
                key={ticket.id}
              >
                <div className="support-ticket-head">
                  <span>
                    ...
                    <small>
                      {canManage ? `${ticket.organization_name} · ` : ""}
                      {categories[ticket.category]} · דחיפות {priorities[ticket.priority]} ·{" "}
                      {new Date(ticket.created_at).toLocaleDateString("he-IL")}
                    </small>
                  </span>
                  <span
                    className={`badge ${["resolved", "closed"].includes(ticket.status) ? "success" : ticket.priority === "urgent" ? "critical" : "warning"}`}
                  >
                    {statuses[ticket.status]}
                  </span>
                </div>
                <p className="card-muted">{ticket.description}</p>
                {ticket.resolution_note ? (
                  <p>
                    <strong>עדכון טיפול:</strong> {ticket.resolution_note}
                  </p>
                ) : null}
                {canManage ? (
                  <div className="support-manager-controls">
                    <label className="field">
                      <span>עדכון ללקוח</span>
                      <textarea
                        className="textarea"
                        value={notes[ticket.id] ?? ""}
                        onChange={(event) => setNotes((current) => ({ ...current, [ticket.id]: event.target.value }))}
                        placeholder="מה בוצע או איזה מידע נוסף דרוש?"
                      />
                    </label>
                    <div className="actions">
                      ...
                      <button
                        className="button"
                        disabled={!!busy}
                        onClick={() => void updateTicket(ticket, "waiting_customer")}
                      >
                        ממתינה ללקוח
                      </button>
                      <button
                      ...
                      >
                        {busy === ticket.id ? <Loader2 className="spin" size={16} /> : <CheckCircle2 size={16} />} סימון
                        כנפתרה
                      </button>
                      <button
