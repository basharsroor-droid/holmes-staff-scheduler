...
export function DepartmentsClient({
  ...
  const permittedBranches =
    callerRole === "manager" && callerBranchId ? branches.filter((branch) => branch.id === callerBranchId) : branches;
  ...
  const branchMembers = members.filter(
    (member) => member.branch_id === branchId && !["owner", "admin"].includes(member.role)
  );
  ...
  async function createDepartment() {
    ...
    const { data, error } = await supabase
      .from("departments")
      .insert({ organization_id: organizationId, branch_id: branchId, name: name.trim() })
      .select("id, branch_id, name, active")
      .single();
    setCreating(false);
    if (error || !data) {
      setMessage(
  ...
  async function toggleDepartment(department: Department) {
    ...
    const { error } = await supabase
      .from("departments")
      .update({ active: !department.active, updated_at: new Date().toISOString() })
      .eq("id", department.id)
  ...
  async function setMemberDepartment(member: Member, departmentId: string, checked: boolean) {
    ...
    setBusyId(member.id);
    setMessage("");
    const { error } = await supabase.rpc("set_membership_departments", {
      target_membership_id: member.id,
      target_department_ids: nextIds
    });
    setBusyId(null);
    if (error) {
    ...
    <div className="departments-workbench business-setup-workbench">
      <section className="template-form-card departments-sidebar setup-sidebar">
        ...
        {canCreate ? (
          <>
            <div>
              <p className="eyebrow">מחלקה חדשה</p>
              <h2>הוספה לסניף</h2>
            </div>
            <label className="field">
              <span>שם המחלקה</span>
              <input
                className="input"
                placeholder="לדוגמה: קבלה"
                value={name}
                onChange={(event) => setName(event.target.value)}
              />
            </label>
            <button className="button primary" disabled={creating || !branchId} onClick={() => void createDepartment()}>
              {creating ? <Loader2 className="spin" size={17} /> : <Plus size={17} />} יצירת מחלקה
            </button>
          </>
        ) : null}
      ...
      <div className="departments-content">
        <section className="template-list-card departments-panel">
          <div className="template-list-heading setup-list-heading">
            <div>
              <p className="eyebrow">{permittedBranches.find((branch) => branch.id === branchId)?.name}</p>
              <h2>{branchDepartments.length} מחלקות</h2>
            </div>
            <span className="status-chip active">
              <Building2 size={14} /> סניף פעיל
            </span>
          </div>
          <div className="department-grid">
            {branchDepartments.map((department) => (
              <article className={`department-card ${department.active ? "" : "inactive"}`} key={department.id}>
                <span className="template-icon">
                  <Network />
                </span>
                <span>
                  <strong>{department.name}</strong>
                  <small>{department.active ? "פעילה וניתנת לשיוך" : "מושבתת — ההיסטוריה נשמרת"}</small>
                </span>
                {canCreate ? (
                  <button
                    className="button"
                  ...
                  >
                    {busyId === department.id ? <Loader2 className="spin" size={16} /> : <Power size={16} />}
                    {department.active ? "פעילה" : "כבויה"}
                  </button>
                ) : (
                  <span className="status-chip active">
                    <Check size={14} /> פעילה
        ...
        <section className="template-list-card department-access-panel">
          <div className="template-list-heading setup-list-heading">
            <div>
              <p className="eyebrow">הרשאות הצוות</p>
              <h2>שיוך למחלקות</h2>
            </div>
            <span className="status-chip active">
          ...
          <div className="department-members">
            {branchMembers.map((member) => {
              const memberAssignments = new Set(
                assignments.filter((item) => item.membership_id === member.id).map((item) => item.department_id)
              );
              const displayName = member.profile
                ? `${member.profile.first_name} ${member.profile.last_name}`.trim()
                : "חבר/ת צוות";
              return (
                <article className="department-member" key={member.id}>
                  <div className="department-member-head">
                    ...
                    <span>
                      ...
                      <small>
                        ...
                        {member.access_scope === "branch"
                          ? "כל הסניף"
                          : member.access_scope === "department"
                            ? "מחלקות נבחרות"
                            : "נתונים אישיים"}
                      </small>
                    </span>
                    ...
                    {branchDepartments
                      ...
                      .map((department) => (
                        <label className="check-field" key={department.id}>
                          <input
                            type="checkbox"
                            checked={memberAssignments.has(department.id)}
                            disabled={busyId === member.id}
                            onChange={(event) => void setMemberDepartment(member, department.id, event.target.checked)}
                          />
                          <span>
                            <strong>{department.name}</strong>
                          </span>
