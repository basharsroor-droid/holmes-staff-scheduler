...
export default async function DepartmentsPage() {
  ...
  const [organizationResult, branchesResult, departmentsResult, membershipsResult, assignmentsResult] =
    await Promise.all([
      ...
      supabase
        .from("organization_memberships")
        .select("id, user_id, branch_id, role, status, access_scope")
        .eq("organization_id", membership.organization_id)
        .eq("status", "active")
        .order("created_at"),
      supabase
        .from("department_memberships")
        .select("department_id, membership_id, is_primary")
        .eq("organization_id", membership.organization_id)
    ]);
  if (!organizationResult.data) redirect("/workspace");

  const visibleMemberships = (membershipsResult.data ?? []).filter(
    (item) =>
      ["owner", "admin"].includes(membership.role) ||
      Boolean(membership.branch_id && item.branch_id === membership.branch_id)
  );
  ...
  return (
    <main className="workspace-home" dir="rtl">
      ...
      <DepartmentsClient
        ...
        callerBranchId={membership.branch_id}
