    ...
    <main className="workspace-home" dir="rtl">
      ...
      <section className="template-list-card audit-console">
        <div className="template-list-heading audit-heading">
          <div>
            <p className="eyebrow">מעקב מאובטח</p>
            ...
            <table className="table compact-table">
              ...
              <tbody>
                {(events ?? []).map((event) => {
                  ...
                  return (
                    <tr className={`audit-row operation-${operation}`} key={event.id}>
                      <td>
                        {new Date(event.created_at).toLocaleString("he-IL", { dateStyle: "short", timeStyle: "short" })}
                      </td>
                      <td>
                        {event.actor_user_id ? (profileMap.get(event.actor_user_id) ?? "משתמש מערכת") : "פעולת מערכת"}
                      </td>
                      <td>
                        <span className="badge">{operationLabels[operation] ?? operation}</span>
                      </td>
