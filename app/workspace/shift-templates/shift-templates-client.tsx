...
export function ShiftTemplatesClient({
  ...
  const [departmentId, setDepartmentId] = useState(
    departments.find((item) => item.branch_id === selectedBranchId)?.id ?? ""
  );
  ...
  async function toggleTemplate(template: ShiftTemplate) {
    ...
    setTemplates((current) =>
      current.map((item) => (item.id === template.id ? { ...item, active: nextActive } : item))
    );
  }
  ...
  return (
    <section className="template-workbench business-setup-workbench shift-template-workbench">
      <div className="template-form-card setup-form-card shift-template-form">
        <div>
          <p className="eyebrow">משמרת חדשה</p>
          <h2>הוספת סוג משמרת</h2>
        </div>
        {branches.length > 1 ? (
          <label className="field">
            <span>סניף</span>
            <select className="input" value={branchId} onChange={(event) => selectBranch(event.target.value)}>
              {branches.map((branch) => (
        ...
        <div className="form-pair">
          ...
          <label className="field">
            ...
            <input
              ...
              onChange={(event) =>
                setForm((current) => ({ ...current, requiredEmployees: Number(event.target.value) || 1 }))
              }
            />
          </label>
        </div>
        <label className="check-field">
          <input
            type="checkbox"
            checked={form.requiresSenior}
            onChange={(event) => setForm((current) => ({ ...current, requiresSenior: event.target.checked }))}
          />
          <span>
        ...
        <button className="button primary" disabled={busy || !branchId || !departmentId} onClick={addTemplate}>
          {busy ? <Loader2 className="spin" size={17} /> : <Plus size={17} />} שמירת משמרת
        </button>
      ...
      <div className="template-list-card setup-list-card shift-template-list-card">
        <div className="template-list-heading setup-list-heading">
          <div>
            <p className="eyebrow">
              {branches.find((branch) => branch.id === branchId)?.name ?? "הסניף"} ·{" "}
              {branchDepartments.find((department) => department.id === departmentId)?.name ?? "מחלקה"}
            </p>
            <h2>
              {departmentTemplates.length ? `${departmentTemplates.length} סוגי משמרות` : "עדיין לא הוגדרו משמרות"}
            </h2>
          </div>
        ...
        <div className="template-list">
          {departmentTemplates.map((template) => (
            <article
              className={`template-item shift-template-row ${template.active ? "" : "inactive"}`}
              key={template.id}
            >
              ...
              <div className="template-meta">
                <span>
                  <Users size={15} /> {template.required_employees} עובדים
                </span>
                {template.requires_senior_employee ? (
                  <span>
                    <ShieldCheck size={15} /> עובד בכיר
                  </span>
                ) : null}
              </div>
              <button className="button" aria-pressed={template.active} onClick={() => void toggleTemplate(template)}>
                <Power size={16} /> {template.active ? "פעילה" : "כבויה"}
              </button>
            </article>
          ))}
