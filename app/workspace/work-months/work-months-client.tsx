...
export function WorkMonthsClient({
  ...
  function selectBranch(branchId: string) {
    setForm((current) => ({
      ...current,
      branchId,
      departmentId: departments.find((department) => department.branch_id === branchId)?.id ?? ""
    }));
  }
  ...
  return (
    <div className="template-workbench business-setup-workbench work-months-workbench">
      <section className="template-form-card setup-form-card work-month-form">
        ...
        <div className="form-pair">
          <label className="field">
            <span>סניף</span>
            <select className="input" value={form.branchId} onChange={(e) => selectBranch(e.target.value)}>
              {branches.map((branch) => (
          ...
          <label className="field">
            ...
            <input
              ...
              max={2100}
              value={form.year}
              onChange={(e) => setForm((c) => ({ ...c, year: Number(e.target.value) }))}
            />
          </label>
        </div>
        <label className="field">
        ...
        <label className="field">
          ...
          <select
            className="input"
            value={form.month}
            onChange={(e) => setForm((c) => ({ ...c, month: Number(e.target.value) }))}
          >
        ...
        <label className="field">
          <span>פתיחת ההגשה</span>
          <input
            className="input"
            type="datetime-local"
            value={form.opensAt}
            onChange={(e) => setForm((c) => ({ ...c, opensAt: e.target.value }))}
          />
        </label>
        <label className="field">
          ...
          <input
            ...
            type="datetime-local"
            value={form.closesAt}
            onChange={(e) => setForm((c) => ({ ...c, closesAt: e.target.value }))}
          />
        </label>
        <div className="card-muted">
          <strong>{departmentTemplates.length} סוגי משמרות פעילים</strong>
          <p className="auth-secondary">
            {departmentTemplates
              .map((template) => `${template.name} ${template.start_time.slice(0, 5)}–${template.end_time.slice(0, 5)}`)
              .join(" · ") || "לא הוגדרו משמרות"}
          </p>
        </div>
        <button className="button primary" disabled={busy} onClick={() => void createPeriod()}>
          {busy ? <Loader2 className="spin" size={17} /> : <CalendarPlus size={17} />} פתיחת חודש עבודה
        </button>
      ...
      <section className="template-list-card setup-list-card work-months-list">
        <div className="template-list-heading setup-list-heading">
          <div>
            <p className="eyebrow">תקופות עבודה</p>
        ...
        <div className="template-list">
          {periods.map((period) => {
            ...
            return (
              <article
                className={`template-item work-month-card period-${period.status} ${period.status === "archived" ? "inactive" : ""}`}
                key={period.id}
              >
                ...
                <div className="template-main">
                  ...
                  <span>
                    {branch?.name ?? "סניף"} · {department?.name ?? "מחלקה"}
                  </span>
                </div>
                ...
                {["collecting", "draft"].includes(period.status) ? (
                  <button
                    className="button"
                    disabled={busyId === period.id}
                    onClick={() => void changeStatus(period, collecting ? "draft" : "collecting")}
                  >
                    {busyId === period.id ? (
                    ...
                    {collecting ? "סגירת הגשה" : "פתיחה מחדש"}
                  </button>
                ) : null}
              </article>
            );
