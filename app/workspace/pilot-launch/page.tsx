...
export default async function PilotLaunchPage() {
  ...
  const [{ data: organization }, { data: managerDepartments }] = await Promise.all([
    supabase
      .from("organizations")
      .select("name, schedule_cadence, pilot_mode")
  ...
  const [
    ...
    { count: pilotFeedbackCount },
    { count: baselineBeforeCount },
    { count: baselineAfterCount }
  ] = await Promise.all([
    periodQuery,
    supabase
    ...
    supabase
      ...
      .select("id", { count: "exact", head: true })
      .eq("organization_id", membership.organization_id)
      .like("subject", "[Pilot Feedback]%"),
    supabase
      .from("support_tickets")
      .select("id", { count: "exact", head: true })
      .eq("organization_id", membership.organization_id)
      .like("subject", "[Pilot Baseline - Before]%"),
    supabase
      .from("support_tickets")
      .select("id", { count: "exact", head: true })
      .eq("organization_id", membership.organization_id)
      .like("subject", "[Pilot Baseline - After]%")
  ]);

  const periods = (periodRows ?? []) as Period[];
  const activePeriod =
    periods.find((period) => ["collecting", "draft", "published"].includes(period.status)) ?? periods[0] ?? null;
  ...
  const employeeCount = (activeMemberships ?? []).filter(
    (row) => row.role === "employee" && scopedMembershipIds.includes(row.id)
  ).length;
  ...
  const submissionCompletion =
    employeeCount > 0 ? Math.min(100, Math.round((submittedCount / employeeCount) * 100)) : 0;
  const requiredAssignments = (shifts ?? []).reduce(
    (sum, shift) => sum + Math.max(0, shift.required_employees ?? 0),
    0
  );
  ...
  const timeToSchedule =
    activePeriod?.submission_closes_at && activePeriod?.published_at
      ? Math.max(
          0,
          Math.round(
            (new Date(activePeriod.published_at).getTime() - new Date(activePeriod.submission_closes_at).getTime()) /
              60_000
          )
        )
      : null;
    ...
    <main className="workspace-home" dir="rtl">
      <header className="workspace-subheader">
        <div>
          ...
          <p className="eyebrow">
            {organization.name} · השקת פיילוט{organization.pilot_mode ? " · Pilot Mode פעיל" : ""}
          </p>
          <h1>
        ...
        <div className="workspace-stats schedule-stats">
          <article>
            ...
            <span>
              ...
              <small>התקדמות להשקה</small>
            </span>
          </article>
          <article>
            <CalendarCheck />
            <span>
              <strong>{submissionCompletion}%</strong>
              <small>השלמת הגשות</small>
            </span>
          </article>
          <article>
            <Gauge />
            <span>
              ...
              <small>כיסוי שיבוצים</small>
            </span>
          </article>
          <article>
            <Clock3 />
            <span>
              <strong>{durationLabel(timeToSchedule)}</strong>
              <small>Time to Schedule</small>
            </span>
          </article>
        </div>
      </section>
      ...
      <section className="pilot-readiness" aria-labelledby="pilot-launch-steps">
        <div className="pilot-readiness-head">
          ...
          <div className="pilot-progress" aria-label={`${progress}% הושלמו`}>
            <strong>{progress}%</strong>
            <small>
              {completedSteps} מתוך {launchSteps.length}
            </small>
          </div>
        </div>
      ...
      <section className="workspace-next workspace-next-primary">
        <div>
          <p className="eyebrow">פעולות מנהל</p>
          <h2>הצעד הבא במחזור הראשון</h2>
        </div>
        <div className="workspace-actions">
          <Link href="/workspace/submissions">
            <ClipboardCheck />
            <span>
          ...
          <Link href="/workspace/pilot-baseline">
            <Timer />
            <span>
          ...
          <Link href="/workspace/pilot-feedback">
            <MessageSquareText />
            <span>
      ...
      {cycleOneDone ? (
        <section className="submission-banner open">
          <div>
            <CheckCircle2 />
