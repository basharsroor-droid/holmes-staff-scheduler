...
export function MyShiftsClient({ periods, branches, shifts }: { periods: Period[]; branches: Branch[]; shifts: Shift[] }) {
  ...
  return <div className="grid">
    {upcoming ? <section className="template-list-card"><div className="template-list-heading"><div><p className="eyebrow">המשמרת הקרובה</p><h2>{upcoming.name}</h2></div><span className="badge success"><CheckCircle2 size={15} /> משובצ/ת</span></div><div className="workspace-stats" style={{ margin: 0 }}><article><CalendarDays /><span><strong>{new Date(`${upcoming.shift_date}T12:00:00`).toLocaleDateString("he-IL", { weekday: "long", day: "numeric", month: "long" })}</strong><small>תאריך</small></span></article><article><Clock3 /><span><strong>{upcoming.start_time.slice(0,5)}–{upcoming.end_time.slice(0,5)}</strong><small>שעות</small></span></article><article><MapPin /><span><strong>{branches.find((branch) => branch.id === periods.find((item) => item.id === upcoming.schedule_period_id)?.branch_id)?.name ?? "הסניף"}</strong><small>מיקום</small></span></article></div>{upcoming.manager_note ? <div className="submission-banner open" style={{ marginTop: 18 }}><div><strong>הערת מנהל</strong><span>{upcoming.manager_note}</span></div></div> : null}</section> : null}
    <section className="template-list-card">
      <div className="template-list-heading"><div><p className="eyebrow">הסידור שלי</p><h2>{period ? `${monthNames[period.month - 1]} ${period.year}` : ""}</h2></div><select className="input" style={{ maxWidth: 280 }} aria-label="בחירת חודש" value={selectedPeriodId} onChange={(event) => setSelectedPeriodId(event.target.value)}>{periods.map((item) => <option value={item.id} key={item.id}>{monthNames[item.month - 1]} {item.year} · {branches.find((branch) => branch.id === item.branch_id)?.name ?? "סניף"}</option>)}</select></div>
      <div className="workspace-stats" style={{ margin: "0 0 20px" }}><article><CalendarDays /><span><strong>{visibleShifts.length}</strong><small>משמרות בחודש</small></span></article><article><Clock3 /><span><strong>{totalHours.toFixed(totalHours % 1 ? 1 : 0)}</strong><small>שעות מתוכננות</small></span></article><article><MapPin /><span><strong>{branches.find((branch) => branch.id === period?.branch_id)?.name ?? "הסניף"}</strong><small>סניף</small></span></article></div>
      <div className="template-list">{visibleShifts.map((shift) => <article className="card" key={shift.id}><div className="mini-row" style={{ border: 0, padding: 0 }}><div className="template-icon"><CalendarDays /></div><span><strong>{new Date(`${shift.shift_date}T12:00:00`).toLocaleDateString("he-IL", { weekday: "long", day: "numeric", month: "long" })}</strong><small>{shift.name}</small></span><span className="badge success">{shift.start_time.slice(0,5)}–{shift.end_time.slice(0,5)}</span></div>{shift.manager_note ? <p className="card-muted" style={{ marginTop: 12 }}>{shift.manager_note}</p> : null}</article>)}</div>
