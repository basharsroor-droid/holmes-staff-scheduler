...
function createIcsEvent(shift: Shift, branchName: string) {
  ...
  const endDate =
      ...
      : shift.shift_date;
  const description = ["ShiftPilot", shift.manager_note ? `הערת מנהל: ${shift.manager_note}` : null]
    .filter(Boolean)
    .join("\\n");
...
export function MyShiftsClient({
  ...
  const initialPeriod =
    periods.find((period) => shifts.some((shift) => shift.schedule_period_id === period.id && shiftStart(shift) >= now))
      ?.id ??
    periods.at(-1)?.id ??
    "";
  ...
  const upcoming = shifts
    .filter((shift) => shiftStart(shift) >= now)
    .sort((a, b) => shiftStart(a).getTime() - shiftStart(b).getTime())[0];
  ...
  return (
    <div className="grid my-shifts-dashboard">
      {upcoming ? (
        <section className="template-list-card next-shift-card">
          <div className="template-list-heading next-shift-heading">
            <div>
              <p className="eyebrow">המשמרת הקרובה</p>
              <h2>{upcoming.name}</h2>
            </div>
          ...
          <div className="workspace-stats next-shift-stats">
            <article>
              ...
              <span>
                <strong>
                  {new Date(`${upcoming.shift_date}T12:00:00`).toLocaleDateString("he-IL", {
                    weekday: "long",
                    day: "numeric",
                    month: "long"
                  })}
            ...
            <article>
              ...
              <span>
                <strong>
                  {upcoming.start_time.slice(0, 5)}–{upcoming.end_time.slice(0, 5)}
                </strong>
                <small>שעות</small>
              </span>
            </article>
            <article>
              ...
              <span>
                <strong>
                  {branches.find(
                    (branch) => branch.id === periods.find((item) => item.id === upcoming.schedule_period_id)?.branch_id
                  )?.name ?? "הסניף"}
                </strong>
                <small>מיקום</small>
              </span>
          ...
          {upcoming.manager_note ? (
            <div className="submission-banner open" style={{ marginTop: 18 }}>
              <div>
                <strong>הערת מנהל</strong>
                <span>{upcoming.manager_note}</span>
              </div>
            </div>
          ) : null}
        </section>
      ...
      <section className="template-list-card my-schedule-card">
        <div className="template-list-heading my-schedule-toolbar">
          <div>
            <p className="eyebrow">הסידור שלי</p>
            <h2>{period ? `${monthNames[period.month - 1]} ${period.year}` : ""}</h2>
          </div>
          <div className="actions">
            <select
              className="input my-schedule-period-select"
            ...
            <button
              ...
              onClick={() =>
                downloadCalendar(
                  ...
                  branches,
                  periods,
                  `shiftpilot-${period?.year ?? "schedule"}-${String(period?.month ?? "").padStart(2, "0")}.ics`
                )
              }
            >
        ...
        <div className="workspace-stats my-schedule-stats">
          <article>
            <CalendarDays />
            <span>
          ...
          <article>
            <Clock3 />
            <span>
              <strong>{totalHours.toFixed(totalHours % 1 ? 1 : 0)}</strong>
              <small>שעות מתוכננות</small>
            </span>
          </article>
          <article>
            <MapPin />
            <span>
              <strong>{branches.find((branch) => branch.id === period?.branch_id)?.name ?? "הסניף"}</strong>
              <small>סניף</small>
            </span>
          </article>
        </div>
        <div className="template-list my-shift-list">
          {visibleShifts.map((shift) => {
            const branchName = branches.find((branch) => branch.id === period?.branch_id)?.name ?? "הסניף";
            return (
              <article className="card my-shift-card" key={shift.id}>
                <div className="mini-row my-shift-row">
                ...
                {shift.manager_note ? <p className="card-muted my-shift-note">{shift.manager_note}</p> : null}
                <div className="actions">
                  <button
