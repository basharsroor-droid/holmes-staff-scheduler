...
function notificationCopy(templateKey: string, payload: unknown, branches: Map<string, string>) {
  ...
  if (templateKey === "shift_reminder")
    return {
      ...
      details: scheduleDetails(payload, branches),
...
export default async function NotificationsPage() {
  ...
  const [{ data: organization }, { data: notifications }, { data: branches }, { data: preferences }] =
    await Promise.all([
      ...
      supabase
        ...
        .select("schedule_published, shift_changes, shift_reminders, availability_reminders, swap_updates")
        .eq("organization_id", membership.organization_id)
        .eq("user_id", user.id)
        .maybeSingle()
    ]);
  ...
  const initialPreferences: PreferenceValues = preferences ?? {
  ...
  return (
    <main className="workspace-home notifications-page" dir="rtl">
      ...
      <section className="template-list-card notifications-center">
        <div className="template-list-heading notifications-heading">
          <div>
            <p className="eyebrow">מרכז עדכונים</p>
            <h2>{unreadCount ? `${unreadCount} התראות חדשות` : "הכול מעודכן"}</h2>
          </div>
        ...
        <div className="template-list notifications-list">
            ...
            return (
              ...
              >
                <div className="template-icon">
                  {notification.template_key === "schedule_published" ? <CalendarCheck /> : <Bell />}
                </div>
                <span>
                ...
                <Link className="button" href={copy.href}>
                  {notification.read_at ? "פתיחה" : "צפייה"}
                </Link>
              </article>
            );
          })}
