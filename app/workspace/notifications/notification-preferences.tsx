...
export function NotificationPreferences({
  ...
  return (
    <section className="template-list-card notification-preferences-card" aria-labelledby="email-preferences-title">
      <div className="template-list-heading notification-preferences-heading">
        <div>
          <p className="eyebrow">שליטה אישית</p>
          <h2 id="email-preferences-title">
            <Mail size={20} /> העדפות מייל
          </h2>
        </div>
        ...
        {labels.map((item) => (
          <label className="notification-preference" key={item.key}>
            ...
            <input
              ...
              aria-label={`קבלת מייל: ${item.title}`}
            />
          </label>
        ))}
      </div>
      {message ? (
        <p className={`form-message ${kind}`} role="status">
          {message}
        </p>
      ) : null}
    </section>
