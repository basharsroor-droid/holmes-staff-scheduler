...
export default async function ManagerCommandCenterPage() {
  ...
  if (membership.role === "manager" && !departmentIds.length) {
    return <main className="workspace-home" dir="rtl"><header className="workspace-subheader"><div><Link href="/workspace" className="back-link"><ArrowRight size={17} /> חזרה לסביבת העבודה</Link><p className="eyebrow">Manager Command Center</p><h1><Activity /> מרכז שליטה למנהל</h1><p>אין לך כרגע מחלקה פעילה לניהול.</p></div></header></main>;
  }
  ...
  const { data: shiftsData } = activePeriod
    ? await (supabase as any).from("shifts").select("id, required_employees, open_for_requests, status").eq("schedule_period_id", activePeriod.id).neq("status", "cancelled")
    : { data: [] };
  ...
  const [{ data: assignmentsData }, { data: marketplaceRequests }, { data: leaveRequests }, { data: swapRequests }] = await Promise.all([
    shiftIds.length
      ...
      : Promise.resolve({ data: [] }),
    shiftIds.length
      ? (supabase as any).from("open_shift_requests").select("id, shift_id").eq("status", "pending").in("shift_id", shiftIds)
      : Promise.resolve({ data: [] }),
  ...
  for (const row of assignments) assignmentCounts.set(row.shift_id, (assignmentCounts.get(row.shift_id) ?? 0) + 1);
  ...
  return <main className="workspace-home" dir="rtl">
    <header className="workspace-subheader"><div>
      ...
      <p className="eyebrow">{organization.name} · Phase 3</p>
      <h1><Activity /> Manager Command Center</h1>
