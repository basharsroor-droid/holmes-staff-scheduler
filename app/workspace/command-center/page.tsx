...
function periodStatusLabel(status: string) {
  return (
    ({ collecting: "פתוח להגשת זמינות", draft: "טיוטת סידור", published: "פורסם" } as Record<string, string>)[status] ??
    status
  );
}
...
export default async function ManagerCommandCenterPage() {
  ...
  if (membership.role === "manager" && departmentIds.length)
    periodQuery = periodQuery.in("department_id", departmentIds);
  if (membership.role === "manager" && !departmentIds.length) {
    return (
      <main className="workspace-home" dir="rtl">
        <header className="workspace-subheader">
          <div>
            ...
            <p className="eyebrow">מרכז שליטה למנהל</p>
            <h1>
  ...
  const activePeriod =
    periods.find((period) => ["draft", "published", "collecting"].includes(period.status)) ?? periods[0] ?? null;
  const { data: shiftsData } = activePeriod
    ? await supabase
        .from("shifts")
        .select("id, required_employees, open_for_requests, status")
        .eq("schedule_period_id", activePeriod.id)
        .neq("status", "cancelled")
    : { data: [] };
  const shifts = (shiftsData ?? []) as Shift[];
  const shiftIds = shifts.map((shift) => shift.id);

  const [{ data: assignmentsData }, { data: marketplaceRequests }, { data: leaveRequests }, { data: swapRequests }] =
    await Promise.all([
      shiftIds.length
        ? supabase.from("shift_assignments").select("shift_id").in("shift_id", shiftIds)
        : Promise.resolve({ data: [] }),
      shiftIds.length
        ? supabase.from("open_shift_requests").select("id, shift_id").eq("status", "pending").in("shift_id", shiftIds)
        : Promise.resolve({ data: [] }),
      supabase
        .from("leave_requests")
        .select("id")
  ...
  for (const row of (assignmentsData ?? []) as Assignment[])
  ...
  const totalAssigned = shifts.reduce(
    (sum, shift) => sum + Math.min(shift.required_employees, assignmentCounts.get(shift.id) ?? 0),
    0
  );
  const coverage = totalRequired ? Math.round((totalAssigned / totalRequired) * 100) : null;
  const understaffed = shifts.filter(
    (shift) => (assignmentCounts.get(shift.id) ?? 0) < shift.required_employees
  ).length;
  ...
  const actions = [
    understaffed > 0
      ...
      : null,
    !pilotMode && pendingMarketplace > 0
      ? {
    ...
    <main className="workspace-home" dir="rtl">
      <header className="workspace-subheader">
        <div>
          ...
          <p className="eyebrow">{organization.name} · תמונת מצב ניהולית</p>
          <h1>
            <Activity /> מרכז שליטה למנהל
          </h1>
      ...
      {!activePeriod ? (
        ...
        <>
          <section className="template-list-card">
            <div className="template-list-heading">
              <div>
                ...
                <h2>{monthLabel(activePeriod.year, activePeriod.month)}</h2>
                <p>סטטוס: {periodStatusLabel(activePeriod.status)}</p>
              </div>
              <Link className="button" href="/workspace/schedule-builder">
                <CalendarRange size={16} /> לסידור המלא
              </Link>
            </div>
            <div className="workspace-stats schedule-stats">
              ...
              <article>
                ...
                <span>
                  ...
                  <small>החלטות שממתינות למנהל</small>
                </span>
              </article>
            </div>
          </section>
          <section className="workspace-next workspace-next-primary">
            <div>
              <p className="eyebrow">תיבת החלטות</p>
              <h2>מה מחכה להחלטה שלך</h2>
            </div>
            <div className="workspace-actions">
              {!pilotMode && (
                <Link href="/workspace/open-shifts">
                  <Store />
                  <span>
              ...
              <Link href="/workspace/schedule-builder">
                <Palmtree />
                <span>
              ...
              <Link href="/workspace/shift-swaps">
                <Repeat2 />
                <span>
          ...
          <section className="template-list-card">
            ...
            {actions.length ? (
              <div className="template-list">
                {actions.map((action) => (
                  <Link className="template-item" href={action.href} key={action.label}>
                    <div className="template-main">
                      <strong>{action.label}</strong>
                      <span>{action.detail}</span>
                    </div>
                    <span className={`badge ${action.priority}`}>לטיפול</span>
                  </Link>
                ))}
              </div>
            ) : (
          ...
          {!pilotMode && (
            <section className="workspace-next workspace-next-compact">
              <div>
                <p className="eyebrow">בדיקות חכמות</p>
                <h2>בדיקה עמוקה ותיקון</h2>
              </div>
              <div className="workspace-actions">
                <Link href="/workspace/schedule-builder">
                  <Gauge />
