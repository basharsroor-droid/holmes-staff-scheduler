...
export default async function ManagerCommandCenterPage() {
  ...
  const pendingTotal = pendingMarketplace + pendingLeave + pendingSwaps;

  const actions = [
    understaffed > 0 ? { href: "/workspace/schedule-builder", label: `לטפל ב-${understaffed} משמרות עם כיסוי חסר`, detail: "פתחו את בדיקת הקונפליקטים או את Fix My Schedule וקבלו תוכנית תיקון מוסברת.", priority: "critical" } : null,
    ...
    pendingLeave > 0 ? { href: "/workspace/schedule-builder", label: `לבדוק ${pendingLeave} בקשות חופשה או היעדרות`, detail: "אישור בקשת היעדרות הופך מיד לאילוץ קשיח בסידור.", priority: "warning" } : null,
  ...
  return <main className="workspace-home" dir="rtl">
    ...
    {!activePeriod ? <section className="template-list-card"><div className="empty-template-state"><CalendarRange size={44} /><h2>עדיין לא נפתחה תקופת עבודה</h2><p>כדי לקבל תמונת מצב אמיתית על כיסוי, בקשות והמלצות פעולה, פתחו קודם תקופת עבודה שבועית או חודשית.</p><Link className="button primary" href="/workspace/work-months"><PlusCircle size={16} /> פתיחת תקופת עבודה</Link></div></section> : <>
      ...
      <section className="workspace-next workspace-next-compact"><div><p className="eyebrow">בדיקות חכמות</p><h2>בדיקה עמוקה ותיקון</h2></div><div className="workspace-actions"><Link href="/workspace/schedule-builder"><Gauge /><span><strong>ShiftPilot Score ובדיקת קונפליקטים</strong><small>ציון בריאות, הוגנות, Smart Draft, Fix My Schedule ו־Smart Replacement.</small></span><span className="status-chip active">פתיחה</span></Link></div></section>
    </>}
  </main>;
}
