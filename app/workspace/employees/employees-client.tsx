...
export function EmployeesClient({
  ...
  async function updateEmployee(
    id: string,
    changes: Partial<Pick<Employee, "role" | "status" | "can_open" | "can_close" | "weekly_hours_limit">>
  ) {
    setMessage("");
    setBusyId(id);
  ...
  async function resendInvitation(invitation: Invitation) {
    ...
    setInvitations((current) =>
      current.map((item) => (item.id === invitation.id ? { ...item, token, expires_at: expiresAt } : item))
    );
  ...
  async function transferOwnership(employee: Employee) {
    ...
    if (
      !window.confirm(
        `להעביר את הבעלות על העסק ל${name}? התפקיד שלך יירד ל"מנהל מערכת" מיד לאחר מכן. הפעולה בלתי הפיכה מהצד שלך לבד — רק הבעלים החדש יוכל להעביר בחזרה.`
      )
    )
      return;
    setMessage("");
  ...
  return (
    <div className="template-workbench employees-workbench">
      <section className="template-form-card employee-invite-panel">
        <div>
          <p className="eyebrow">עובד חדש</p>
        ...
        <div className="form-pair">
          ...
          <label className="field">
            ...
            <select
              className="input"
              value={form.role}
              onChange={(e) => setForm((c) => ({ ...c, role: e.target.value as Role }))}
            >
        ...
        <button className="button primary" disabled={inviteBusy} onClick={() => void inviteEmployee()}>
          {inviteBusy ? <Loader2 className="spin" size={17} /> : <MailPlus size={17} />} שליחת הזמנה
        </button>
        ...
        <div>
          ...
          <div className="template-list">
            {invitations.slice(0, 6).map((invitation) => {
              ...
              return (
                <article
                  className={`template-item invitation-item ${invitation.status !== "pending" ? "inactive" : ""}`}
                  key={invitation.id}
                >
      ...
      <section className="template-list-card employee-directory">
        <div className="template-list-heading employee-directory-heading">
          <div>
            <p className="eyebrow">הצוות של העסק</p>
            <h2>{employees.length} חברי צוות</h2>
          </div>
        ...
        <div className="template-list employee-directory-list">
          {employees.map((employee) => {
            ...
            return (
              <article
                className={`template-item employee-row ${employee.status === "suspended" ? "inactive" : ""}`}
                key={employee.id}
              >
                <div className="template-icon" style={{ color: employee.profile?.color }}>
                  <UserRound />
                </div>
                <div className="template-main">
                  <strong>
                ...
                <div className="actions">
                    ...
                    <button
                      ...
                      onClick={() =>
                        void updateEmployee(employee.id, {
                          status: employee.status === "suspended" ? "active" : "suspended"
                        })
                      }
                    >
                  ...
                  {callerRole === "owner" && !isSelf && employee.role !== "owner" && employee.status === "active" ? (
                    <button
                      ...
                      onClick={() => void transferOwnership(employee)}
                    >
                      {busyId === employee.id ? <Loader2 className="spin" size={16} /> : <Crown size={16} />} העברת
                      בעלות
                    </button>
                  ) : null}
                </div>
                  ...
                  <div className="employee-controls">
                    <label className="field">
                      ...
                      <select
                        ...
                        disabled={isSelf || busyId === employee.id || employee.role === "owner"}
                        value={employee.role}
                        onChange={(e) => void updateEmployee(employee.id, { role: e.target.value as Role })}
                      >
                        <option value="employee">עובד/ת</option>
                        <option value="manager">מנהל/ת</option>
                        <option value="admin">מנהל מערכת</option>
                    ...
                    <label className="check-field">
                      <input
                        type="checkbox"
                        checked={employee.can_open}
                        disabled={busyId === employee.id}
                        onChange={(e) => void updateEmployee(employee.id, { can_open: e.target.checked })}
                      />
                      <span>
                        <strong>פתיחה</strong>
                      </span>
                    ...
                    <label className="check-field">
                      <input
                        type="checkbox"
                        checked={employee.can_close}
                        disabled={busyId === employee.id}
                        onChange={(e) => void updateEmployee(employee.id, { can_close: e.target.checked })}
                      />
                      <span>
                        <strong>סגירה</strong>
                      </span>
                    ...
                    <label className="field">
                      ...
                      <input
                        ...
                        disabled={busyId === employee.id}
                        value={hoursDraft[employee.id] ?? ""}
                        onChange={(e) => setHoursDraft((current) => ({ ...current, [employee.id]: e.target.value }))}
                        onBlur={() => void commitHoursLimit(employee)}
                      />
                    </label>
                  </div>
                ) : null}
