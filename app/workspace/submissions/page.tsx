...
export default async function SubmissionsPage() {
  ...
  const [
    ...
    supabase
      ...
      .select("id, branch_id, department_id, year, month, status, submission_closes_at")
      .eq("organization_id", membership.organization_id)
  ...
  const [{ data: profiles }, { data: submissions }] = await Promise.all([
    ...
    periodIds.length
      ? supabase
          ...
          .select("id, schedule_period_id, user_id, submitted_at, manager_note, updated_at")
  ...
  const { data: entries } = submissionIds.length
    ? await supabase
        ...
        .select("id, submission_id, shift_template_id, shift_date, status, note")
