...
export function SubmissionsClient({
  ...
  const [notes, setNotes] = useState<Record<string, string>>(() =>
    Object.fromEntries(initialSubmissions.map((item) => [item.id, item.manager_note ?? ""]))
  );
  ...
  return (
    <section className="template-list-card submissions-console">
      <div className="template-list-heading submissions-toolbar">
        <div>
          <p className="eyebrow">חודש, סניף ומחלקה</p>
          <h2>{period ? `${monthNames[period.month - 1]} ${period.year}` : ""}</h2>
        </div>
        <select
          className="input submissions-period-select"
      ...
      <div className="actions submission-filters" role="group" aria-label="סינון הגשות">
        {(
          ...
          <button
            className={`button ${filter === value ? "primary" : ""}`}
            aria-pressed={filter === value}
            onClick={() => setFilter(value)}
            key={value}
          >
            {label}
            {value === "draft" ? ` (${draftCount})` : ""}
          </button>
        ))}
      </div>
      {period ? (
        <div className={`submission-banner submission-deadline ${period.status === "collecting" ? "open" : "closed"}`}>
          <Clock3 />
          <div>
            <strong>
      ...
      <div className="template-list submission-list">
        {visibleWorkers.map((worker) => {
          ...
          return (
            <article className={`card submission-worker-card ${state}`} key={worker.id}>
              <div className="mini-row submission-worker-head">
                <div className="submission-worker-identity">
                  <div className="template-icon" style={{ color: worker.profile?.color }}>
                    <UserRound />
                  </div>
                  <span>
                    <strong>{name}</strong>
                    <small>
                      {worker.role === "manager" ? "מנהל/ת" : "עובד/ת"} · {submissionEntries.length} סימונים
                    </small>
                  </span>
                </div>
                <span
                  className={`badge ${state === "submitted" ? "success" : state === "draft" ? "warning" : "critical"}`}
                >
                  {state === "submitted" ? "הוגש" : state === "draft" ? "טיוטה" : "חסר"}
                </span>
                {submission ? (
                  <button className="button" onClick={() => setExpandedUserId(expanded ? null : worker.user_id)}>
                    {expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />} פירוט
                  </button>
                ) : null}
              </div>
              {expanded && submission ? (
                <div className="grid submission-details">
                    ...
                    <table className="table compact-table">
                      ...
                      <tbody>
                        {submissionEntries.map((entry) => {
                          const template = templates.find((item) => item.id === entry.shift_template_id);
                          return (
                            <tr key={entry.id}>
                              <td>{new Date(`${entry.shift_date}T12:00:00`).toLocaleDateString("he-IL")}</td>
                              <td>
                                {template?.name ?? "משמרת"}{" "}
                  ...
                  <label className="field submission-manager-note">
                    <span>הערת מנהל פנימית</span>
                    <textarea
                      className="textarea"
                  ...
                  <button
                    ...
                    onClick={() => void saveManagerNote(submission)}
                  >
                    {busyId === submission.id ? <Loader2 className="spin" size={16} /> : <Save size={16} />} שמירת הערה
                  </button>
                </div>
              ) : null}
            </article>
