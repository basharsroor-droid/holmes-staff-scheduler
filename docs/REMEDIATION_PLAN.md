...
**Waves 0–3 הושלמו, מלבד B3 (הבא בתור) ו-F1 (בבדיקה, #268).** D1 רץ לראשונה מול staging ב-11.9 ומצא **שני באגים אמיתיים בפרודקשן** — שניהם תוקנו ואומתו (#266, #267). Wave 4 התחיל: J3 הושלם, I1 ו-I2 חלקיים; J1/J2 נדחו בהחלטת בעלים. "אומת" = נבדק בפועל בפרודקשן, ב-staging או ב-CI, לא רק מוזג.
...
| 2 | **F1** מחיקת מסלול הדמו הישן | 🔄 בבדיקה | #268 | החלטת בעלים: `/demo` = סיור סטטי במוצר → ניסיון חינם, **בלי חשבון דמו משותף בפרודקשן**; 38 קבצים נמחקו |
...
**מה חוסם כרגע:** שום דבר ב-Waves 0–3. B3 הבא בתור; F1 ממתין לבדיקות ולבדיקה ויזואלית של `/demo`. Wave 4 (הכנסה) ממתין להחלטת הבעלים על ספק הסליקה (J1).
