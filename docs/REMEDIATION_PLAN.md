...
| — | באנר ניסיון שפג | ✅ | #259 | |
| 4 | **J1/J2** סליקה + ניהול מנוי | 🟡 תשתית מוכנה | #278 | שכבת `BillingGateway` ניטרלית לספק, חישוב מחיר חודשי/שנתי + מבצע השקה, עמודות ספק ב-`subscriptions`, טבלת `billing_events` (service-role בלבד, RLS ללא policies). **אין checkout, אין webhook חי ואין חיוב.** נשאר: חיבור ספק (החלטת בעלים) + מסך ניהול מנוי; Vercel Pro לפני הלקוח המשלם הראשון |
