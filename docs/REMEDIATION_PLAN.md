...
> `shiftHours`, `bounds`, `overlaps`, `weekStartKey` משוכפלים ב-4 פאנלים לפחות. אותו קוד, העתק-הדבק.
- **ביצוע:** חילוץ למודול יחיד + unit tests (כולל משמרות חוצות חצות, שבוע שמתחיל ביום ראשון, שעון קיץ)
- **קבלה:** 0 הגדרות כפולות; כיסוי unit על משמרת חוצת-חצות ומעבר שעון
...
> "מנוחה מינימלית" ו"שעות שבועיות" קיימים גם כ-triggers ב-SQL וגם כלוגיקת דירוג ב-TS. **שני מקורות אמת.** זה מקובל (SQL = אכיפה, TS = חיזוי/UX) — אבל לא מתועד, אז שינוי בצד אחד ישבור שקט את השני.
- **ביצוע:** טבלה ב-`docs/coverage-rules.md`: חוק → איפה נאכף → איפה נחזה → מה קורה כשהם לא מסכימים
- **קבלה:** כל חוק ב-`shift-validation.ts` מופיע בטבלה
