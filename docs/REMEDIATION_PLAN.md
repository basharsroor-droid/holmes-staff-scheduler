...
**Waves 0–3 הושלמו, מלבד שני פריטים** שממתינים למפתח ה-staging (F1, B3). Wave 4 (הכנסה) טרם התחיל. "אומת" = נבדק בפועל בפרודקשן, ב-staging או ב-CI, לא רק מוזג.
...
| 1 | **D1** E2E על `/workspace` מול staging | ⏸️ מוזג, **מדלג** | #235 | ממתין ל-`STAGING_SUPABASE_SECRET_KEY` |
...
| 2 | **B3** הגבלת טעינת ה-schedule-builder | ⏸️ **ממתין ל-D1** | #247 (תכנון) | נמדד מחדש: 27 שאילתות, טוען את כל ההיסטוריה |
| 2 | **F1** מחיקת מסלול הדמו הישן | ⏸️ **ממתין ל-D1** | — | |
...
**מה חוסם כרגע:** הגדרת `STAGING_SUPABASE_SECRET_KEY` (בעלים) משחררת את D1, ובעקבותיו את F1 ו-B3.
