...
| 4 | **I1** אשף הקמה | 🟡 חלקי | #263 | אירועי פאנל + view `private.setup_funnel`; נותרו הדרכה בתוך כל צעד ותבניות לפי סוג עסק |
| — | **I2** מצבי ריק ושגיאה | ✅ | #264, I2-ErrorState | `<EmptyState>` ב-17 קבצים; שגיאות דרך `StatusMessage kind="error"` (אייקון אחר + `role="alert"`) — נותרו 0 קבצים עם הודעת מצב מקומית; קודם כשלונות הוצגו בבאנר ירוק עם ✓ |
