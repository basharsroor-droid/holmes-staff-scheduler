...
| — | באנר ניסיון שפג | ✅ | #259 | |
| 4 | **J1/J2** סליקה + ניהול מנוי | 🟡 תשתית מוכנה | #278, #282, #284 | שכבת `BillingGateway` ניטרלית לספק, חישוב מחיר חודשי/שנתי + מבצע השקה, עמודות ספק ב-`subscriptions`, טבלת `billing_events` (service-role בלבד, RLS ללא policies). **אין checkout, אין webhook חי ואין חיוב.** מסך "המנוי שלי" לקריאה בלבד פעיל (#282): מסלול, ניסיון, מחיר ושימוש מול כל מכסה. דף תשלום (#284) בנוי: בחירת מסלול ומחזור, סיכום מ-`quoteInvoice` כולל מבצע ההשקה, אזהרת חריגה ממכסה — **כפתור התשלום מושבת**. נשאר: חיבור ספק (החלטת בעלים) לכפתור + שדרוג/ביטול; Vercel Pro לפני הלקוח המשלם הראשון |
