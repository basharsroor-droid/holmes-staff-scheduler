...
**Waves 0–3 הושלמו, מלבד B3 (בעבודה: B3a ב-#271).** D1 רץ לראשונה מול staging ב-11.9 ומצא **שני באגים אמיתיים בפרודקשן** — שניהם תוקנו ואומתו (#266, #267). Wave 4 התחיל: J3 הושלם, I1 ו-I2 חלקיים; J1/J2 נדחו בהחלטת בעלים. "אומת" = נבדק בפועל בפרודקשן, ב-staging או ב-CI, לא רק מוזג.
...
| 2 | **B3** הגבלת טעינת ה-schedule-builder | ⏭️ **הבא בתור** (D1 ירוק) | #247 (תכנון) | נמדד מחדש: 27 שאילתות, טוען את כל ההיסטוריה |
...
| — | **I2** מצבי ריק ושגיאה | 🟡 חלקי | #264 | `<EmptyState>` ב-17 קבצים; נותר `<ErrorState>` (12 קבצים עם `setMessage` גולמי) |
...
**מה חוסם כרגע:** שום דבר ב-Waves 0–3. B3 בעבודה (B3a ב-#271, B3b אחריו). Wave 4 (הכנסה) ממתין להחלטת הבעלים על ספק הסליקה (J1).
