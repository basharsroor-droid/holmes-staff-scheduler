...
**Waves 0–3 הושלמו, מלבד שלושה פריטים** (שניים ממתינים למפתח ה-staging, אחד נדחה בכוונה). Wave 4 (הכנסה) טרם התחיל. "אומת" = נבדק בפועל בפרודקשן, ב-staging או ב-CI, לא רק מוזג.
...
| 3 | **F2** פירוק JSX צפוף | 🔜 פתוח | — | 377 שורות >200 תווים; ייעשה כשאין PRים פתוחים |
