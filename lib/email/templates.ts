...
export function notificationEmail(templateKey: string, payload: Record<string, unknown>, appUrl: string): EmailTemplate {
  ...
  const templates: Record<string, EmailTemplate> = {
    ...
    support_ticket_created: { subject: `פניית תמיכה חדשה: ${text(payload.subject, "ללא כותרת")}`, preview: "פנייה חדשה מחכה לצוות התמיכה", heading: "התקבלה פניית תמיכה חדשה", body: `${text(payload.subject, "פנייה חדשה")} · דחיפות ${text(payload.priority, "רגילה")}`, cta: "פתיחת מרכז התמיכה", href: "/workspace/support" },
