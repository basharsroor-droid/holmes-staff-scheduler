...
export function notificationEmail(templateKey: string, payload: Record<string, unknown>, appUrl: string): EmailTemplate {
  ...
  const templates: Record<string, EmailTemplate> = {
    ...
    support_ticket_created: { subject: `פניית תמיכה חדשה: ${text(payload.subject, "ללא כותרת")}`, preview: "פנייה חדשה מחכה לצוות התמיכה", heading: "התקבלה פניית תמיכה חדשה", body: `${text(payload.subject, "פנייה חדשה")} · דחיפות ${text(payload.priority, "רגילה")}`, cta: "פתיחת מסוף התמיכה", href: "/support" },
    support_ticket_updated: { subject: `עדכון בפניית התמיכה: ${text(payload.subject, "הפנייה שלך")}`, preview: "צוות התמיכה של ShiftPilot עדכן את הפנייה", heading: "פניית התמיכה עודכנה", body: text(payload.resolution_note, "סטטוס הטיפול בפנייה שלך עודכן."), cta: "צפייה בפנייה", href: "/workspace/support" }
  };
...
// Colors match the brand tokens documented in docs/BRAND_GUIDE.md
// (--primary #2158c9, --primary-dark #163f9e, --ink #172033, --muted
// #667085, --line #d8e0ea, --bg #f5f7fb) -- these three email surfaces
// (this renderer, invite.html, recovery.html) used to carry an older,
// undocumented blue (#2f6fed/#142640) that predated the brand guide and
// had drifted from the site's actual colors. White-on-#2158c9 is also a
// real contrast improvement, not just a rebrand: 6.36:1 vs the old
// #2f6fed's 4.55:1, which sat close enough to the WCAG AA line to be
// worth moving away from regardless.
export function renderEmail(template: EmailTemplate, appUrl: string) {
  const href = new URL(template.href, appUrl).toString();
  return `<!doctype html><html dir="rtl" lang="he"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(template.subject)}</title></head><body style="margin:0;background:#f5f7fb;font-family:Arial,sans-serif;color:#172033"><div style="display:none;max-height:0;overflow:hidden">${escapeHtml(template.preview)}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:32px 16px"><table role="presentation" width="100%" style="max-width:600px;background:#fff;border-radius:22px;border:1px solid #d8e0ea;overflow:hidden"><tr><td style="padding:26px 32px;background:#163f9e;color:#fff;font-size:24px;font-weight:800">ShiftPilot</td></tr><tr><td style="padding:38px 32px;text-align:right"><p style="margin:0 0 10px;color:#667085;font-size:14px">ניהול משמרות חכם ופשוט</p><h1 style="margin:0 0 18px;font-size:28px">${escapeHtml(template.heading)}</h1><p style="margin:0 0 28px;line-height:1.8;font-size:16px">${escapeHtml(template.body)}</p><a href="${escapeHtml(href)}" style="display:inline-block;background:#2158c9;color:#fff;text-decoration:none;padding:13px 22px;border-radius:12px;font-weight:700">${escapeHtml(template.cta)}</a></td></tr><tr><td style="padding:20px 32px;background:#f8fafc;color:#667085;font-size:12px;text-align:right">הודעה זו נשלחה לפי העדפות ההתראות שלך ב-ShiftPilot.</td></tr></table></td></tr></table></body></html>`;
}
