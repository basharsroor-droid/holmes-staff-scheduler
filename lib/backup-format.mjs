...
// Format 1 (until 2026-09-11): the first 23 public tables. Format 2: every
// public table plus auth.users (lib/backup-tables.mjs). Same checksum.
...
export function validateBackupPayload(payload, expectedTables) {
  ...
  if (!SUPPORTED_FORMAT_VERSIONS.includes(payload.format_version)) throw new Error(`Unsupported backup format version: ${payload.format_version}`);
  ...
  return { integrity: "verified", formatVersion: payload.format_version, rowCounts: payload.row_counts };
}
