-- Baseline schema, captured retroactively.
--
-- These objects (types, the 14 foundation tables, their indexes, the
-- private.is_org_member/has_org_role helpers, create_organization_workspace,
-- RLS, and grants) were applied directly to the ShiftPilot Supabase project
-- before migration tracking started -- they exist on Production today but
-- were never recorded as a tracked migration. That gap breaks every
-- "replay migrations from empty" path: a fresh Supabase branch, a
-- disaster-recovery restore to a clean project, or a new environment all
-- fail immediately because the very first tracked migration
-- (20260807172343_harden_privileged_auth_rpcs) already assumes
-- public.organizations exists.
--
-- This file is a verbatim capture of db/supabase-scheduler-schema.sql (the
-- documented foundation schema), timestamped before that first tracked
-- migration so a fresh replay creates these objects, then continues
-- unmodified into the real tracked history. It is recorded as
-- already-applied on Production (a metadata-only row in
-- supabase_migrations.schema_migrations) rather than re-run there, since
-- the objects already exist -- see docs/RUNBOOK.md for how and why.
--
-- Columns added to these tables by later tracked migrations (organizations:
-- min_rest_hours, is_demo, schedule_cadence, pilot_mode; shifts:
-- open_for_requests/opened_at/opened_by; schedule_periods/shift_templates:
-- department_id; organization_memberships: weekly_hours_limit,
-- access_scope; notifications: read_at) are intentionally NOT included
-- here -- they are added by their own already-tracked ALTER migrations that
-- run after this one.
