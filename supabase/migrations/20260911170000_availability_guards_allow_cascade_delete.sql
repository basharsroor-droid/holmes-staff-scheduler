-- Account deletion was blocked for any business whose employees had submitted
-- availability.
--
-- /api/account/delete (App Store guideline 5.1.1(v)) deletes the owner's
-- organization with the service role. The delete cascades into
-- availability_submissions and availability_entries, whose guard triggers
-- start with "if auth.uid() is null then raise 'Authentication required'" --
-- and a service-role request has no user. So the organization delete failed
-- and the owner got "Could not delete organization". The same guard made every
-- staging E2E fixture cleanup fail silently (it only warned), which is how it
-- was found: 2026-09-11, while deleting leftover staging test data.
--
-- A delete that arrives through a foreign-key cascade -- the whole
-- organization, schedule period or user is being removed -- is not an
-- employee editing their availability, so the guards let it through. Such a
-- delete fires the guard from inside the referential-integrity trigger, so
-- pg_trigger_depth() > 1. A direct DELETE on these tables (depth 1) is guarded
-- exactly as before. Everything else in both functions is unchanged.
...
begin
  -- Cascade from deleting the organization, schedule period or user.
...
begin
  -- Cascade from deleting the submission, organization, schedule period or user.
  if tg_op = 'DELETE' and pg_trigger_depth() > 1 then return old; end if;
